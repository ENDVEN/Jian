...
### 数据源坑（§11.5-98 / -99 · v6.68 · 东财源评估 · 从 `JIAN_RULES.md` 搬入 · **口径仍现行**）

   - **11.5-98** 【v6.68 · 数据源】**"外部源整片被拒"时用户会先怀疑自己 ⇒ 界面必须说出来**：
     M2 行业列长期全 '—'（用户："是不是我下载没弄好？"）—— 实测**代码与下载都没错**：是**东财
     `clist/get` 端点对本机直接断连**（同域名单点报价却 HTTP 200；新浪源全正常；无代理）。**口径**：
     ① 细分行业唯一来源就是它 ⇒ 不通就永远空、`industry_map.json` 从不落盘；② **抓取失败必须出声**
     （写明"未取到 + 再扫一次会重试"），旧版该分支静默 ⇒ 用户只会怀疑自己；③ **诊断三连**：同域名单点
     接口对照 + 换源对照（新浪）+ 查代理 ⇒ 一眼分清"源挂了 / 代理 / 代码"。
   - **11.5-99** 【v6.68 · 数据源】**东财 `push2/push2his` 是"概率性风控"，不是我们能修的 bug**（用户
     2026-09-26 立项深查，因为它决定"以后还依不依赖东财"）。**实测四证**：① **同端点相隔 20 分钟结论
     相反**（`clist` 先 200、后立即断开；`stock/get` 先 200、后全挂）；② **换 UA / Referer / Cookie
     全无影响**（裸头 200 那次，带浏览器头反而挂）；③ 失败都在 **0.7s 内被服务端 RST**（不是超时、不是
     参数错）；④ 同一刻**网页 `quote.eastmoney.com`（51KB）与 `datacenter-web`（4.4KB）稳定 200** ⇒
     **不是断网 / 代理 / IP 全封，而是按"IP + 端点 + 频次"的概率性拦截**。**社区同向**：akshare GitHub
     #6784 定性"**目标服务器限制了 IP**，一个 IP 多次访问就不返回"；2025–26 多篇"被限流 / 封 IP / 弹
     验证码"实践报告（其中一篇的"根因分析"实为引流广告文，正文零技术细节，**不作为依据**）。
     **口径**：① 东财只当**单次请求型第二源**（全市场快照 / 估值），**永不承担"几十次连击"型任务**
     —— 行业映射旧的 `板块清单 + 逐板块成分`＝**80+ 连击**正是风控最敏感的模式，故它**几乎从没成功过**；
     ② 高频取数要么换**极少请求**的入口、要么换源；③ 取不到一律诚实降级 + **出声**（v6.68 已做）；
     ④ ⚠ **探测/诊断本身也会把 IP 打进风控**（我连打 30 余次后，全网关从"部分 200"变成"全挂"，
     静等 150s 未恢复、换 `push2delay` / plain http 主机同样挂）⇒ 排查要**省着打、间隔开**，
     别信"多试几次就好了"；⑤ 候选改法（`clist` 的 `f100`＝所属行业字段，一次/几页取全市场行业）
     **本轮未验成**（正撞上被自己打出的风控窗）⇒ **待验证**，别当既成事实。
     **⚠ 修正（同日 18:20–18:55 复测，**推翻**上面那句"网页登录无改善"）**：那句是**误判** —— 我做的
     两次"复测"用的都是**匿名**请求（网页 GET **不向 Python 会话下发 Cookie**，`cookies=[]`），所以
     "登录后仍失败"说明不了登录没用。用户按方案 1 用扩展**导出 Cookie**后：**带真 Cookie 的同端点请求
     立刻 200、`f100` 有值**，而**同一刻的匿名请求仍被 RST** ⇒ **门槛是"登录态"，不是"无差别封 IP"。**
     **最终结论（同日 4 次实测）**：东财 `push2` 对**匿名**请求有**频次窗** —— 连打几十次 ⇒ 该端点对匿名
     **整段拒绝**（0.7s 内 RST）；**静置约 30 分钟自动恢复**（18:55 匿名复测回到 200 ✓）；**带登录
     Cookie 可豁免**（限额外期间亦能通过）。⇒ **产品取舍：不把登录做成依赖** —— 匿名 + 低频 + 长缓存即可。
     **`f100` 实测**：`clist` 的 `f100`＝**所属行业**（样例 `301311→通用设备`、`688682→军工电子Ⅱ`，口径
     与旧"东财行业板块"一致）；**单页上限 = 100 行**（请求 pz=1000 也只回 100）⇒ 全市场 ~5560 只
     = **~56 页**。**据此的改造（v6.68 已落地）**：行业映射**放弃**"板块清单 + 逐板块成分（80+ 连击）"，
     改为**同端点分页直取 `f100`**：每批 8 页 + 页间隔 0.5s + 进度存 `preferences['industry_fetch']`
     （**断点续抓**）+ 分批 `merge()` 入库（空值不覆盖）⇒ 分几次扫描摊平；补齐后长期缓存。
     **诊断纪律**：探测本身会烧掉匿名额度 ⇒ 省着打、间隔开。

### 近期坑（§11.5-93 / -94 / -95 · v6.66–v6.68 从 `JIAN_RULES.md` 搬入 · **口径仍现行**）
   ...
   - **11.5-94** 【v6.67 · 数据】**"抄近路的旁路"会把新加的校验整段绕过去**：`_try_apply_spot` 命中即
     `return`（v1.45 为省请求而设），而 v1.47 的"复权因子漂移 ⇒ 整段重算"在主路径**之后** ⇒ 除权当天
     历史永不重算（用户实测："当天转赠或分红的股票抓到的是不复权"）。**口径**：① 给主路径加校验必须
     回头找**所有"提前 return"的旁路**并同步加闸门；② 旁路判据要**零成本且精确**（快照"昨收" vs 本地
     末根收盘）；③ **判据缺失一律不放行**；④ 判据列**只当判据**（`prev_close` 不进湖白名单）。
   - **11.5-95** 【v6.67 · 测试】**冒烟偶发 hard-crash（`0xC0000409`）≠ 代码回归**：同一份代码 6 次里 1 次
     崩在"会真起 worker"的分节（机器上有别的进程抢 CPU（如**用户正在跑的应用本体**）时，QThread 收尾撞上
     对象销毁 ⇒ Qt fastfail，无 Python 栈）。**判据**：换时间重跑即绿 ⇒ 环境/时序，别据此改业务代码；
     且验证前先 `Get-Process python` —— 用户的应用进程**绝不许当测试残留杀掉**（本轮差点误杀）。
     **另两条同源（v6.68 实测）**：① **防污染自检不许只看 mtime** —— 用户开着的应用会合法改写
     `preferences.json` / `trade_calendar.json` ⇒ 假红；判据升级为**内容指纹**（指纹一致 ⇒ 未污染；
     指纹变了就比"我们可能写的那几个键"是否逐值一致）。② `smoke_pages_overlay` 的 M2「点数值列排序」
     断言曾因用户真实 `scan_ui.col_order` 而**稳定假红**（视觉列→逻辑列错位）⇒ 起手抹掉 `col_order`。
     **通用口径：冒烟与"用户正在用应用"共存时，任何与外部写者相关的判据都必须以"内容/结构"为准，
     而不是时间戳或次数。**

### v6.68 更新块（v6.68 从 `JIAN_RULES.md` §11.6 搬入 · **数字冻结**）

> ✅ **已落地（v6.68 / `1.50` · 「切不复权图不变」= 空字符串 key 被吞 · 2026-09-25 用户实测）**
> 用户实测："数据管理里 601825 的前复权与不复权**都重新全量下载了**，可回行情页**无论怎么切、图都是
> 前复权**"。**探针先切开两头**：数据层**是好的**（两份首根 7.65 / 10.70 且 1236/1237 根不同）、程序
> 入口**也是好的**（`select_adjust('')` 后首根 10.70）⇒ 真因在**真实点击**那条路：
> `SegmentedControl._on_clicked` 用 `not key` 判"无效段"，而**「不复权」的业务值 `ADJUST_NONE` 就是
> 空字符串** ⇒ 手点被整段吞掉（状态不动、高亮也不动）；`select_adjust()` 走 `set_current()`（**没有**
> 那个真值判断）⇒ 一切正常 ⇒ **所有程序入口断言全绿，只有真人点不动**。**修法**：改**按索引**判有效
> （`key_at()` 越界也返回 `""` ⇒ 靠返回值分不出"越界"与"合法空 key"）+ 断言补**真实点击**。
> **同批（测试隔离 2 处）**：① 防污染自检从 mtime 改**内容指纹**（用户开着的应用会合法改写
> `preferences.json` / `trade_calendar.json` ⇒ 旧判据假红）；② 抹掉用户真实 `scan_ui.col_order`
> （视觉列→逻辑列整体错位 ⇒ 点"换手率"落在"状态"列 ⇒ M2 排序断言稳定假红）。
> **同批（流程 2 条 · 用户 2026-09-26 拍板）**：③ **提交备注必须短**（照 1.12–1.33：首词 + 2–4 个短词组、
> ≤40 字、禁套话）落 **§9-A 末条** + 机器护栏（读 HEAD 的 `%s`）；④ 同一版本号被提交多次（实测
> `1.47`×2 / `1.48`×3）用 `git commit-tree` 合并成一条（不碰工作区、逐字节校验树一致，坑 = §11.5-97）。
> 坑 = **§11.5-96 / -97**。断言 `smoke_pages_overlay` **714 → 718**，`smoke_chart` 869 不变。

### v6.67 更新块（v6.68 从 `JIAN_RULES.md` §11.6 搬入 · **数字冻结**）

> ✅ **已落地（v6.67 / `1.49` · 云端同步一次两份 + 复权正确性再补一刀 · 2026-09-25 用户实测驱动）**
> ① **「云端同步」改成一次点齐两份**（用户拍板："必须要求按下后同时下载前复权与不复权的数据"）：
>   `DeskData.SYNC_ZONES` 两个分区**各起一个** worker（`SingleSyncGate` 单占位 ⇒ 按分区各一个实例），
>   **收齐才收尾**（按钮恢复用 `_sync_seen` 计数、含被竞态丢弃的回包）；汇总回执分别点名两份；
>   只渲染**当前口径**那份；分钟视图退化为单档。
> ② **M1/M2/M3「更新到最新」拿到"不复权数据"的真因 = spot 秒补旁路绕过了复权自愈**（用户实测："当天
>   转赠或分红的股票抓取的数据都是不复权的"）：`_try_apply_spot` **命中即 return** ⇒ v1.47 的因子漂移
>   检测（在主路径第 3 步）被整段跳过；而除权当天恰好是它最易命中的形态（本地末日 == 上一交易日）
>   ⇒ 只追加当天那根、历史仍按旧因子缩放。**修法**：**除权闸门** `_spot_prev_close_ok()`（快照**昨收**
>   vs 本地末根收盘；`prev_close` 只当判据、不进湖白名单）—— 不等（>0.1%）或判据缺失**一律拒绝秒补**，
>   退回网络增量（带重叠窗口 ⇒ 整段重算）。**正确性优先于省一次请求**。坑 = **§11.5-94**。
> ③ 断言 `smoke_pages_overlay` **699 → 714**、`smoke_chart` **864 → 869**。
> ⚠ **运行纪律**：用户**正在跑的应用**是**启动那一刻的构建** ⇒ 验收前必须**重启应用**（另见 §11.5-95）。

### v6.66 更新块（v6.67 从 `JIAN_RULES.md` §11.6 搬入 · **数字冻结**）

> ✅ **已落地（v6.66 / `1.48` · 不复权分区的下载入口 · 2026-09-25 用户实测驱动）**
> 用户实测（截图）：照"重新全量下载"修完前复权后，**切不复权仍然没数据**；进「🗄 数据管理」一看，
> `kline_daily_raw` **只有 1 项（600326）** —— "我这么久预下载都没有补充这里面的内容"。
> **探针先排除嫌疑**（都只写临时湖）：① `refresh_one(zone=kline_daily_raw)` 真实联网 **1665 行落盘成功**
> （9/21=82.45 → 9/22=57.86 真实跳空）；② 行情页切不复权**真线程端到端**也成功（raw 落盘 + 图表渲染 +
> 回执"已更新，新增 1665 行" + 互斥占位已释放）⇒ **数据层与取数都没坏，坏的是入口**：
> **① 批量预下载没有口径选择**（非指数来源一律写 `kline_daily`）⇒ 预下载几十轮也补不上 raw；
> **② 数据管理页对一个空分区没有任何下载入口**（表里没行可勾 ⇒ 更新/全量按钮**恒灰**）⇒ 死局。
> **修法**：`bulk_download` 加「复权口径」（前复权 / 不复权 / **两个都下**＝两个任务，任务名带口径；
> 指数预设来源该项禁用 + 说明）；`data_manager._sync_targets()` 在**没勾任何行**时回落"过滤框里的代码"
> （只对同步类动作，**删除仍只认勾选**）+ 空分区把出路写进状态行。
> 顺手被自己的断言抓到一个真坑：`str.isalnum()` **对中文也为 True** ⇒ 按名称过滤的"平安"会被当成
> 标的代码去下载 ⇒ 必须加 `isascii()`。坑 = **§11.5-92**。
> 断言 `smoke_pages_overlay` **686 → 699**（+13 入口 +4 发布物 +5 口径可见性），`smoke_chart` **864** 不变（数据层未动）。
