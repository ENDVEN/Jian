...
### 数据源坑（§11.5-98 / -99 · v6.68 · 东财源评估 · 从 `JIAN_RULES.md` 搬入 · **口径仍现行**）
   ...
   - **11.5-99** 【v6.68 · 数据源】**东财 `push2/push2his` 是"概率性风控"，不是我们能修的 bug**（用户
     ...
     **补充实证（同日 · 用户网页端登录后复测）**：同 IP / 同 UA / **同一刻** —— 网页 `quote.eastmoney.com`
     仍 **200** 且 **不下发任何 Cookie**（`cookies=[]`），而 `push2` clist **仍被立即 RST**；用户**在网页端
     登录自己账号后毫无改善** ⇒ **不是"账号 / 会话可信度"问题，而是针对 `push2` 这类 API 端点本身**
     （端点 + 客户端特征 + IP 频次）。⇒ "靠登录提升可信度"对匿名 API 请求**不成立**（除非用同一已登录
     会话的 Cookie，而网页并不下发 ⇒ **无处可带**）。最后一次复核 = 只发 2 个请求（预热 + 接口）、
     失败即停手，避免再次加深风控。
