...
# ⚠ ★v6.74 S2-2 起**已降级：本件没有任何入口**（2026-09-27 用户拍板 S2-2）。
#   唯一编辑面 = **设置页**（左栏最下方「⚙ 设置 → 下载与取数」）；三处旧入口（数据管理页 /
#   预下载弹窗 / 下载队列面板）**全部改为跳到设置页**（见 `ui/views/settings_view.open_group`，
#   §9-D「不新开第二套编辑面」）。
#   本文件**仅作兼容壳**（旧引用 / 旧断言仍可导入），**待 §7-B14 大扫除时删除**。
#   ⚠ 新参数一律进 `ui/settings_registry.py`，**改这里不生效**（没有入口了）。
