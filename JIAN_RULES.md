...
| 项 | 值 |
|---|---|
| APP 版本 | **1.49**（三处同步 §9-A：`APP_VERSION` ✅ / `version.json` ✅ / commit 首词 `1.49` ✅；**push 待用户**） |
| 文档版本 | **v6.67**（云端同步一次两份 + spot 秒补的除权闸门） |
| 最近三版 | `1.49` 云端同步双口径与复权闸门 · `1.48` 不复权分区下载入口 · `1.47` 复权自愈与字体版权收口 |
| **当前主线** | **v6.67 云端同步一次两份 + 复权 spot 闸门**（`1.49` **本地已提交**，**待用户实测 + push（未推送）**）—— ① 「云端同步」同时下前复权 + 不复权；② **除权当天禁止走 spot 秒补**（否则历史不重算 ⇒ 看起来像不复权）。**下一步候选** = §7-B4 回测进阶 / 深色主题 / desk 一键「修回历史」。 |
| **断点** | **v6.67**（用户 2026-09-25 两问："云端同步为什么不抓对应的那份？""M1/M2/M3 更新到最新怎么保证是前复权？"）—— ① 前者按拍板改成**一次同步两份**（两个分区各一个 worker + 各一个互斥件 + 收齐才收尾）；② 后者的真因 = **spot 秒补旁路绕过复权自愈**（除权日"本地末日 == 上一交易日"⇒ 秒补命中 ⇒ 只追加当天、历史不重算）⇒ 新增**除权闸门**（快照"昨收" vs 本地末根收盘，不等或判据缺失即拒绝秒补，退回网络增量整段重算）。坑 = §11.5-94 / §11.5-95。验收两份冒烟全绿（**数字唯一出处 §11.7**）。 |
...
- **业务文件 ≥400 行（★v1.46 现测 = 本轮动过的文件；其余仍是 v1.45 的数。涨落史不抄在这里，
  ...
  ⚠ `data/sync_service.py` **809**（v1.40 日历新鲜度 + v1.43 `failure_hint` + v1.44/1.45 spot 秒补 +
    v1.47 复权自愈 + **v6.67 除权闸门** `_spot_prev_close_ok`；再长把"失败分类与人话文案"抽成
  ...
  `core/backtest.py` **495** > `data/akshare_feed.py` **594**（★v1.44 spot 快照 + ★v6.67 "昨收"判据；
    再加新源前先分文件）>
...
- **11.5-77** 【v1.39 · §7-E3】① **"裁列白名单"必须 apply-if-present（只减不增）**：各分区列集合
   ...
   **用 `git worktree add ..\Jian-baseline HEAD` 拉一份干净基线跑→ 同样 816/1** ⇒ 环境/时机敏感的
   ...
   - **11.5-95** 【v6.67 · 测试】**冒烟偶发 hard-crash（`0xC0000409`）≠ 代码回归**：实测同一份代码 6 次里
     1 次崩在 §7-A4（运行历史）段 —— 那段会**真起 worker**，机器上有别的进程抢 CPU（本轮：**用户正在跑的
     应用本体**）时，QThread 收尾撞上对象销毁 ⇒ Qt fastfail（无 Python 栈、日志停在分节中间）。
     **判据**：① 同一份代码换个时间重跑即绿 ⇒ 环境/时序，不是回归（别据此改业务代码）；
     ② 验证前先 `Get-Process python` —— 用户的应用进程会**一直在**，**绝不许当测试残留杀掉**（本轮差点误杀）。
...
> ⑤⑥（v6.66）：**发布物护栏**（`version.json` 裸引号曾致非法 JSON ⇒ 加机器断言，坑 = §11.5-93）与
> **口径可见性**（M1/M2/M3「更新到最新」写明**口径固定 = 前复权日线**：回测/扫描口径 ≠ 图表口径）。
...
- [ ] **改了行情工作台页面（`trading_desk.py`）/ `ui/widgets/desk_*.py` 任一模块 / 回测页「成交模型」行 / 标注交互层 / **画线类型目录（新增类型、`implemented` 翻牌）**？** → 跑 `py tests/smoke_pages_overlay.py`（**714 项**，含 **§7-B6 的「迁移护栏」+ 顶栏分段控件/分钟档位 + 工具行 chips + 图标轨/分页面板/折起（含**富余宽度归图表、折起后左侧只剩图标轨**两条不变量）+ 读数条 + **口径回执的"除权跳空定位 / 数据体检"**+ STEP 6 的"实现落在哪个 `desk_*.py`"**：公共面被改名、旧入口（`cb_period`/`cb_adjust`/`cmb_tool`）被复活、**把薄壳写成空函数**、**分栏比例退化**、**回执退回"不解释"**，都会立刻红）；
...
- [ ] **改了复盘页（`ui/views/review.py`）/ `ui/widgets/review_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**714 项**，含 **「复盘页迁移护栏」+ §9-U 分栏不变量**：
...
- [ ] **改了全市场筛选页（`ui/views/scan_view.py`）/ `ui/widgets/scan_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**714 项**，含 **「M2 公共面护栏」+ 版式与口径不变量**）。
...
- [ ] **改了广度统计页（`ui/views/breadth_view.py`）/ `ui/widgets/breadth_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**714 项**，含 **「M3 公共面护栏」+ 双窗格/区间/增量不变量**）。
...
- [ ] **改了回测页/工作台/复盘页的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**714 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；四条离屏打桩见 §11.5-20 与 §11.5-79（★第④条 v1.42 补：**异步回测与后台下载也不许写真目录** —— 看到"只有几条真实数据类断言红"先问“有没有东西正在写”），**别删**）
...
- [ ] **改了后台下载（`ui/download_hub.py` / `ui/widgets/download_bar.py` / `download_queue_panel.py`）或四个下载入口（`bulk_download` / `data_manager` / `readiness_flow` / `main_window.downloads`）吗？** → 跑 `py tests/smoke_pages_overlay.py`（**714 项**，含「§7-B11 后台下载」段）。⚠ 四条红线：
