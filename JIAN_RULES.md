...
| 项 | 值 |
|---|---|
| APP 版本 | **1.50**（三处同步 §9-A：`APP_VERSION` ✅ / `version.json` ✅ / commit 首词 `1.50` ✅；**push 待用户**） |
| 文档版本 | **v6.68**（复权切换失效：空字符串 key 被组件吞掉） |
| 最近三版 | `1.50` 复权切换修复 · `1.49` 云端同步双口径与复权闸门 · `1.48` 不复权分区下载入口 |
| **当前主线** | **v6.68 复权切换修复**（`1.50` 已提交；**待强推** + 用户实测）。**下一步候选** = §7-B4 回测进阶 / 深色主题。 |
| **断点** | **v6.68**（用户实测："无论怎么切图都是前复权"）：数据层与程序入口**都正常**，真因 = **`SegmentedControl` 把「不复权」的空字符串 key 当无效吞掉**（手点无效、`set_current` 正常 ⇒ 断言全绿真人点不动）⇒ 改按索引判有效 + 断言补**真实点击**。坑 = §11.5-96。 |
...
- **11.5-77** 【v1.39 · §7-E3】① **"裁列白名单"必须 apply-if-present（只减不增）**：各分区列集合
   ...
   **用 `git worktree add ..\Jian-baseline HEAD` 拉一份干净基线跑→ 同样 816/1** ⇒ 环境/时机敏感的
   ...
   - **11.5-94 / -95**（旁路绕过校验 · 冒烟偶发 fastfail 与"用户正开着应用"共存的判据）→ 详情已搬
     `docs/JIAN_ARCHIVE.md`「三、近期坑（v6.67）」。**仍现行两条口径**：① 给主路径加校验时**必须回头找
     所有"提前 return"的旁路**并同步加闸门，**判据缺失一律不放行**；② 冒烟与"用户正在用应用"共存时，
     任何与**外部写者**相关的判据都以**内容/结构**为准（不是时间戳或次数）；用户的应用进程**绝不许当
     测试残留杀掉**。
   - **11.5-96** 【v6.68 · UI】**"程序入口能过、真人点不动" = 组件把"合法假值 key"当无效吞掉**：
     ...
     **断言全绿、真人点不动**（用户实测："切了不复权图始终是前复权"）。**口径**：① 组件判"无效/缺失"
     **只许用索引 / `in` / `is None`，绝不用真值判断**（`if not x` 会吞 `""`/`0`/`[]` 这类合法值）；
     ② 手点是主入口的控件，断言**必须含一次真实点击**（`buttons()[i].click()`）。
...
> **v6.68 更新块（复权切换失效 + 测试隔离 + 流程 2 条 · `1.50`）→ 已搬 `docs/JIAN_ARCHIVE.md`「三、v6.68」**
> 一句话：真因 = `SegmentedControl` 把「不复权」的**空字符串 key** 当无效吞掉（手点无效、程序入口正常
> ⇒ 断言全绿真人点不动）⇒ 改按索引判有效 + 断言补**真实点击**。同批修两处测试隔离（防污染自检改内容
> 指纹 / 抹掉用户 `scan_ui.col_order`）＋两条流程规范（**备注必须短** §9-A 末条 + 同版本多重提交的合并
> 手法 §11.5-97）。坑 = §11.5-96/97。断言 `smoke_pages_overlay` `**718 项**`、`smoke_chart` 869 不变。

> **v6.67 更新块（云端同步一次两份 + spot 除权闸门 · `1.49`）→ 已搬 `docs/JIAN_ARCHIVE.md`「三、v6.67」**
> 一句话：`SYNC_ZONES` 一次点齐两份（各一个 worker + 各一个互斥件、收齐才收尾）；spot 秒补加**除权闸门**
> （快照昨收 vs 本地末根收盘，不等或判据缺失即拒绝秒补 ⇒ 退回网络增量整段重算）。坑 = §11.5-94/95。
...
- [ ] **改了行情工作台页面（`trading_desk.py`）/ `ui/widgets/desk_*.py` 任一模块 / 回测页「成交模型」行 / 标注交互层 / **画线类型目录（新增类型、`implemented` 翻牌）**？** → 跑 `py tests/smoke_pages_overlay.py`（**718 项**，含 **§7-B6 的「迁移护栏」+ 顶栏分段控件/分钟档位 + 工具行 chips + 图标轨/分页面板/折起（含**富余宽度归图表、折起后左侧只剩图标轨**两条不变量）+ 读数条 + **口径回执的"除权跳空定位 / 数据体检"**+ STEP 6 的"实现落在哪个 `desk_*.py`"**：公共面被改名、旧入口（`cb_period`/`cb_adjust`/`cmb_tool`）被复活、**把薄壳写成空函数**、**分栏比例退化**、**回执退回"不解释"**，都会立刻红）；
...
- [ ] **改了复盘页（`ui/views/review.py`）/ `ui/widgets/review_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**718 项**，含 **「复盘页迁移护栏」+ §9-U 分栏不变量**：
...
- [ ] **改了全市场筛选页（`ui/views/scan_view.py`）/ `ui/widgets/scan_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**718 项**，含 **「M2 公共面护栏」+ 版式与口径不变量**）。
...
- [ ] **改了广度统计页（`ui/views/breadth_view.py`）/ `ui/widgets/breadth_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**718 项**，含 **「M3 公共面护栏」+ 双窗格/区间/增量不变量**）。
...
- [ ] **改了回测页/工作台/复盘页的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**718 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；四条离屏打桩见 §11.5-20 与 §11.5-79（★第④条 v1.42 补：**异步回测与后台下载也不许写真目录** —— 看到"只有几条真实数据类断言红"先问“有没有东西正在写”），**别删**）
...
- [ ] **改了后台下载（`ui/download_hub.py` / `ui/widgets/download_bar.py` / `download_queue_panel.py`）或四个下载入口（`bulk_download` / `data_manager` / `readiness_flow` / `main_window.downloads`）吗？** → 跑 `py tests/smoke_pages_overlay.py`（**718 项**，含「§7-B11 后台下载」段）。⚠ 四条红线：
