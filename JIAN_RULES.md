...
│   ├── smoke_chart.py   # 1407行 图表架构 + 控件/口径/标注/配方/周期(含**分钟**档位)/自选/复权/拖动/坐标轴
...
│   └── smoke_pages_overlay.py # 924行 回测页/工作台叠层 + 标注 + 互送 + 自选/周期(含**分钟**)/
│                        #       复权/坐标轴 + 顶栏分段控件/chips + 图标轨/分页面板/折起
│                        #       （§7-B6 STEP 3b/3c/4）+ 成交模型行/教学弹窗/导出口径**页面级**验收（**225 项**）
