...
>   ⚠ **只提交、未 push**（push 才会给老用户发「✨ 发现新版本」提示）。
...
└── ui/                  # 【表现层：只做展示，禁 SQL/爬虫】(见 §3)
    ...
    │                    #   / backtest_panes.py(420 ★1.22 回测页五张编辑卡片：ƒ函数/⇄条件/📉大盘/🛡风控/
    │                    #        🎯成交（纯视图）+ **EditDrawer 右侧遮罩抽屉**（遮罩 + 页签 +
    │                    #        卡片堆叠）—— 样板 A 的 L3 层；含 mini_label/hint_icon/number_spin
    │                    #        三个表单构件（全 app 唯一来源）。抽屉/遮罩与回测业务无关
    │                    #        ⇒ 其他页面可直接复用（§9-U）
    │                    #        ⚠ 已越 400 线：再长先拆抽屉/卡片两边)
    │                    #   / backtest_result.py(462 ★1.22 结果区（L0 主角）：KPI 四卡 + K线控制行 +
...
- **业务文件 ≥400 行（1.22 实测降序）**：
  ⚠ `ui/views/review.py 1067` > ⚠ `ui/views/trading_desk.py 898` > `data/data_feed.py 827` >
  `ui/views/backtest.py 787`（1.22 拆分收官，**已退出榜首**）> `ui/views/data_manager.py 500` >
  `core/backtest.py 495` > `ui/widgets/backtest_result.py 462` > `core/database.py 435` >
  `ui/widgets/backtest_panes.py 430` > `ui/dialogs/bulk_download.py 416`。
- **验收脚本不参与"业务文件瘦身"**：`tests/smoke_chart.py 1162`、`tests/smoke_pages_overlay.py 736`。
- ✅ **`backtest.py` 1697 → 787（-54%）**：六块各归其位（编辑卡片+抽屉 / 摘要条 / 结果区 /
  导出 / 运行流程 / 策略库）。**剩下的体积债在别的页面**：`review.py`（1067）与
  `trading_desk.py`（898）—— 它们**同时**是 §9-U 的版式收口对象，**动它们时一起办**
  （别为整理而整理）。新能力一律落到 `ui/widgets/*` 或 `data/*_store`（§10-12）。
...
- [x] **代码物理拆分纪律**（v5.13 复核）：
  ...
  - [x] **大文件拆分（1.22 收官）**：`ui/views/backtest.py` 1697 → 1467 → 1156 → **787（-54%）**，
    ...
    统一约定 = **状态留在页面、行为搬进模块**。
    ⚠ 新拆出的 `backtest_result.py(462)` 与 `backtest_panes.py(430)` 已越 400 线，再长先切一刀。
    `review.py 1067` / `trading_desk.py 898` 属**下一批**（§9-L 与 §9-U 一起办）。
...
> 禁止在已有的大页面上继续堆：`backtest.py 1467` / `trading_desk.py 898` / `review.py 1067`（§9-L 体积债）。
...
- **A. ✅ 已修（v5.7）版本号滞后；🔁 编号体系于 v6.16 改版（用户拍板）**：
  ...
  · **当前值 = `1.22`**（= 回测页 UI 重构：摘要条 + 遮罩抽屉；含 §9-L 模块拆分收官）；
    上一轮 `1.21` = §7-B5「回测成交真实性」（三档成交时点 + T+1）；`1.20` = 图表架构 P0–P8
    + 全仓文件归置 + 本版本号体系改版；更早的 `1.4.x` **已废弃**；
...
- **U. 【版式：其他页面未按 §10-14 收口】（1.22 登记 · 长期待办 · 用户 2026-09-16 明确要求
  "把这部分工作写在 rules 里留着，将来再定什么时候做"）**：回测页已于 1.22 收口
  （摘要条 + 遮罩抽屉 + 结果吃满，实测结果区 ≈72%），但**其余页面仍是"所有区块同一优先级
  平铺"**，同样存在"常驻配置挤结果"：
  | 页面 | 现状 | 病灶 |
  |---|---|---|
  | `ui/views/trading_desk.py`(898) | 标的/自选/周期/复权/工具行/画线工具栏/公式编辑器全部常驻图表上方 | 图表（真正长停留的对象）被压成下半屏 |
  | `ui/views/review.py`(1067) | 月/年双模态 + 宏观/微观两段全平铺 | 交易明细与回放图被挤 |
  | `ui/views/data_manager.py`(500) / `dashboard.py` | 分区清点/同步控件常驻 | 轻症，可顺路 |
...
- 3. §9-L 大文件拆分（`backtest.py 1156` 剩下 = **回测流程 / 策略库**，第二批后半 / `review.py 1067` / `trading_desk.py 898`）—— 纯重构，与功能错开。
