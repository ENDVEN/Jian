...
| 项 | 值 |
|---|---|
| APP 版本 | **1.41**（三处同步 §9-A：`APP_VERSION` ✅ / `version.json` ✅ / **commit 首词待用户提交**） |
| 文档版本 | **v6.56**（§7-B1/B2 补漏：M2/M3"需要动作必有入口" + 按钮名单出口 + 切范围清旧结果；用户实测反馈驱动，要求"举一反三"。E1–E5 已提交） |
| 最近三版 | `1.41` §7-B1/B2 补漏（入口完整性） · `1.40` §7-E5 下载层日历感知跳过 · `1.39` §7-E3 日线列白名单 |
| **当前主线** | **§7-E 队列已闭环（E1–E5）；M2/M3 入口完整性补漏已落地 1.41**；**push 待用户**（领先远端 6）。**下一步候选** = §7-B4 回测进阶（复用 `TradeAnalyzer` 绩效维度 + 每回合明细持久化）/ 深色主题（用户原定"功能差不多了再统一做"，四个回测子模块已全部收官，**可能已到该时点**）；**⏸ 另有 1 项待拍板** = 下载速度性能档（并发 / 批量快照通道，见 §11.6 顶部） |
| **断点** | **v6.56（1.41）§7-B1/B2 补漏 ·「需要动作必有入口」✅**：按钮名收成单出口 `custom_widgets.SYNC_ACTION_LABEL` + M2/M3 摘要条**常驻 `btn_sync`**（旧版唯一入口藏在结果区空态里 ⇒ 扫过一次就再也点不到）+ 两页 `_drop_stale_result` 作废旧结果（`ScanResult/BreadthResult.clear`，**同范围重解析不清**）+ **"已有结果"分支绝不调 `set_empty`**（否则把刚扫出的结果藏起来）+ 坏文件补「去数据管理」入口；**旧按钮名彻底退役**（源码级断言钉住）。验收 `smoke_chart` **817** / `smoke_pages_overlay` **566** 全绿。
**前序已闭环**：§7-E5（1.40）下载层日历感知跳过 / §7-E3（1.39）日线列白名单 / §7-E2（1.38）代理说人话+熔断提前 / §7-E4（闸门结案·零代码）/ §7-A4（1.37）回测历史存档 —— 逐条叙事见 §8「最近三版」与 `docs/JIAN_HISTORY.md` §8 全表。
**push 待用户**（领先远端 6）。**⏸ 待拍板**：下载速度性能档（并发 / 批量快照通道）→ 见 §11.6 顶部。 |
...
- **验收**：`smoke_chart` **796 → 803 项** / `smoke_pages_overlay` **566 项** 全绿 + 全仓 compileall。
...
> **1.38 及更早**（`1.38` §7-E2 代理失败说人话+熔断提前 / `1.37` §7-A4 回测历史存档 /
...
> **v6.56（§7-B1/B2 补漏 落地 · `1.41` · 2026-09-23 · 用户实测反馈驱动）**
> 用户报"**M2 统计范围里选任意范围后，体检显示未下载 / 历史不足，但整页没有任何更新入口**"，
> 并明确要求"**不要我提哪里有问题你就改哪里，要举一反三**"。查出的根因**四类**（本次一次性全治）：
> ① **唯一入口依赖结果区状态**：能触发"更新到最新"的只有结果区**空态**按钮，而它只在
>    `_outcome is None`（还没扫过）时出现 ⇒ **一旦扫过一次，整页再也找不到补数据的入口**；
> ② **`resolve_scope` 从不清旧 `_outcome`** ⇒ 范围换成中证500 后，结果区上方还挂着**沪深300 的统计**
>    （命中 6 · 300 只 / 有效样本 295/300），而空态写的是**中证500 的就绪度**（未下载 456/500）
>    —— **两套数字同屏打架**；且 `_render_readiness` 因此误走"已有结果"分支、**不再给入口**；
> ③ **文案指称的按钮已不存在**：v6.47 把"补齐缺失"并入「⬆ 更新到最新交易日」之后，
>    6+ 处文案（`scan_flow` 的二次确认 / `shift_date`、`breadth_flow`、`core/cross_section` 的逐只说明）
>    仍在指引用户去点旧名按钮 ⇒ **指路指到空处**；
> ④ **"需要动作"却"没给动作"**：`_render_readiness` 的"已有结果"分支只 `setText`（无按钮）；
>    "只有坏文件、无缺口"分支也只给一句话（"去数据管理重新全量下载"）而**没有入口**。
> **落地**：
> · **按钮名收成唯一出口** `ui/widgets/custom_widgets.SYNC_ACTION_LABEL`（UI 常量文件，零依赖不成环），
>   各处文案一律引用它 ⇒ **改名即全改**；**`core/` 与 `data/` 不许指名 UI 按钮**（分层纪律），
>   改成"页面的下载入口"。
> · **常驻入口**：M2/M3 摘要条各加 `btn_sync`（同一文字、同一动作）⇒ **任何状态下都有入口**，
>   不再依赖结果区的显示状态。
> · **范围切换作废旧结果**：两页 `_drop_stale_result((范围, 指数代码))` 清 `_outcome` +
>   `ScanResult.clear()` / `BreadthResult.clear()`（KPI / 口径摘要 / 表格复位）⇒ 消除混串；
>   **同一范围重复解析不清**（别白丢用户已跑出来的结果）。
> · **"已有结果"分支绝不调 `set_empty`**：那会 `empty_box.show()` + `table/chart.hide()`，
>   把刚扫出来的结果**藏起来**（比"没按钮"严重得多）—— 入口交给常驻按钮。
> · **有坏文件也属于"需要动作"** ⇒ 补「去数据管理」入口（判据是"要不要用户动手"，不是"有没有缺口"）。
> · **旧按钮名彻底退役**：UI 层源码不许再出现「补齐缺失」；`core/`+`data/` 不许出现按钮全名
>   —— **两条都用源码级断言钉住**（这类漂移肉眼查不出来）。
> 验收：`smoke_pages_overlay` **557→566**（+9：两页常驻按钮都在 / 按钮真接线 / 换范围旧结果作废 /
> 同范围不白丢 / 已有结果不抢结果区 / 坏文件给入口 / 两条源码级防漂移）、`smoke_chart` **817** 全绿
> + 全仓 compileall。坑=**§11.5-80**。
> ✅ **同批完成用户要求的 rules 瘦身（99.1k → 96.8k 字符）**：按 §10-14「先搬走再写新内容」
> 四处压缩：① §11.6 的 v6.49–v6.53 五块 → 3 行归档指针；② §0 快照「断点」行（累积到 2.0k）
> → 当前状态摘要；③ §11.6 v6.54 块 → 5 行；④ §8 收回到"最近三版"。
> **均只删/移"已有归档的复述"，未丢任何决策记录**（逐条都在 `docs/JIAN_HISTORY.md` §8）。
...
- [ ] **改了行情工作台页面（`trading_desk.py`）/ `ui/widgets/desk_*.py` 任一模块 / 回测页「成交模型」行 / 标注交互层 / **画线类型目录（新增类型、`implemented` 翻牌）**？** → 跑 `py tests/smoke_pages_overlay.py`（**566 项**，含 **§7-B6 的「迁移护栏」+ 顶栏分段控件/分钟档位 + 工具行 chips + 图标轨/分页面板/折起（含**富余宽度归图表、折起后左侧只剩图标轨**两条不变量）+ 读数条 + **口径回执的"除权跳空定位 / 数据体检"**+ STEP 6 的"实现落在哪个 `desk_*.py`"**：公共面被改名、旧入口（`cb_period`/`cb_adjust`/`cmb_tool`）被复活、**把薄壳写成空函数**、**分栏比例退化**、**回执退回"不解释"**，都会立刻红）；
...
- [ ] **改了复盘页（`ui/views/review.py`）/ `ui/widgets/review_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**566 项**，含 **「复盘页迁移护栏」+ §9-U 分栏不变量**：
...
- [ ] **改了全市场筛选页（`ui/views/scan_view.py`）/ `ui/widgets/scan_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**566 项**，含 **「M2 公共面护栏」+ 版式与口径不变量**）。
...
- [ ] **改了广度统计页（`ui/views/breadth_view.py`）/ `ui/widgets/breadth_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**566 项**，含 **「M3 公共面护栏」+ 双窗格/区间/增量不变量**）。
...
- [ ] **改了回测页/工作台/复盘页的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**566 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；三条离屏打桩见 §11.5-20，**别删**）
