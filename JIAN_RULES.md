...
| 项 | 值 |
|---|---|
| APP 版本 | **1.27**（= 最近一次 push 的 commit 首词；**唯一出处 §9-A**） |
| 文档版本 | **v6.32**（本文件瘦身 + `docs/` 三册） |
| 最近三版 | `1.27` 文档分层重构 · `1.26` 复盘页收口 + `review`/`data_feed` 拆分 · `1.25` 画线工具重做 |
| **当前主线** | **§7-B1/B2 —— M2 全市场横截面筛选 / M3 广度统计**（设计已定稿，**从 STEP 0 探针开跑**） |
| **断点** | STEP 0 未开始（4 个探针：IO / 面板构建 / 向量化 / 端到端） |
...
| 📐 市场回测 | `BacktestModule` | [~] | M1 单股回测完整（含 **📚 配方库 / 💾 存为配方 / 📤 送到行情页**，v6.11/P7）；M2 全市场筛选 / M3 广度统计为 **ComingSoon 占位**（**v6.31 已立项、设计定稿**：派生面板 `kline_panel` + 一个引擎两种视图 + 三态口径，见 §7-B1/B2 主案） |
...
│   ├── smoke_chart.py   # 1586行 图表架构 + 控件/口径/标注/配方/周期(含**分钟**档位)/自选/复权/拖动/坐标轴
│   │                    #        + **工具行 chips 规则（§7-B6 STEP 3c）**
│   │                    #        + **成交真实性(§7-B5)** + **§7-B8 R7/R8（副图换序 / 纵轴固定左槽）**
│   │                    #        + **§7-B8 R1 自选分组（分类语义 / 删组不删股）**
│   │                    #        + §9-V 数据源护栏（非正价 / 量纲接缝）断言（**589 项**）
...
│   └── smoke_pages_overlay.py # 1082行 回测页/工作台/**复盘页**叠层 + 标注 + 互送 + 自选/周期(含**分钟**)/
│                        #       复权/坐标轴 + 顶栏分段控件/chips + 图标轨/分页面板/折起
│                        #       （§7-B6 STEP 3b/3c/4）+ 成交模型行/教学弹窗/导出口径**页面级**验收（**396 项**）
│                        #       + **「行情工作台 / 复盘页迁移护栏」**（公共面改名/删除、把薄壳写成
...
├── config/
│   └── settings.py      #     全局常量：APP_NAME、APP_VERSION=1.26(跟随 git，与 version.json 同步)、颜色、
...
│   │                    #      ★1.26/§9-L 拆分：827 → **365**（退出 400 线，不再标数字），
│   │                    #      工具/模型/FIFO/月报/月份推断分居下面 4 个新模块，
│   │                    #      公共名**再导出** ⇒ `from data.data_feed import ...` 口径零改动
...
- **业务文件 ≥400 行（1.26 收官实测降序）**：
  ...
  `core/backtest.py 495` > `ui/views/data_manager.py 473` > `ui/widgets/desk_layout.py 570` >
  ...
  （按 v6.16 口径不再标数字）。
- **验收脚本不参与"业务文件瘦身"**：`tests/smoke_chart.py 1407`、
  `tests/smoke_pages_overlay.py 1011`。
...
- ✅ **§9-U/§9-L 剩余体积债已清（1.26）**：`ui/views/review.py` **1067 → 173**（行为分居
  ...
  **`ui/views/` 全部回到 400 线内**。新能力一律落到 `ui/widgets/*` 或 `data/*_store`（§10-12）。
...
- **B1 [~] M2 全市场单日横截面筛选（v6.31 立项 · 待施工）**：`backtest_module.py` `page_scan` 现为
  `_ComingSoonPage`。**主案已定稿**：派生面板 `kline_panel`（日期×标的宽表）+ 两段式漏斗 + 三态口径；
  ⚠ 原写的"需 `scan_cache` zone（公式哈希+日期维缓存）"**已被「L1 面板缓存」取代**（L2 结果缓存后置）。
- **B2 [~] M3 全市场广度家数折线 + 指数叠加（v6.31 立项 · 待施工）**：`page_breadth` 占位；
  指数日线能力已就绪（阶段C：`fetch_index_daily` / `index_daily`）。**与 M2 同一引擎**（逐日求和），
  双轴改用「**上下两窗格 x 联动**」而非真·双 y 轴；`⚡增量到最新` 是本页主路径。
...
- **B7 [ ] ⭐行情工作台**侧边栏**重设计（用户 2026-09-17 指定为后续优化方向 · 待出方案）**：
...
- **B8 [ ] ⭐`1.24` 侧边栏重设计（A 方案）+ 现有功能改进（v6.24 立项 · 用户 2026-09-17 拍板方向）**：
  ...
  **附图纵轴对齐** / 附图数量仍由 chips 控制 / 画线类型**图标+数字** / 支持 30+ 类型 / 数据页暂缓）。
  ⚠ 其中 **R7（副图不可排序）/ R8（纵轴缩进不一致）是代码级真实缺陷**，已核实；
...
| 估值 / 财务数据源 | **未接入**（`valuation` / `fin_report` 是预留空分区；`akshare_feed` 无对应抓取）⇒ **换手率（需流通股本）本轮做不到** |
...
| M2/M3 占位 | `ui/views/backtest_module.py` 的 `page_scan` / `page_breadth` = `_ComingSoonPage` |
...
**D1 · 数据底座 —— 派生面板 `kline_panel`**
- **形态**：`日期 × 标的` 宽表，**按年分区**（约 16 个文件 × ~25–30 MB；**不按日分区** —— 那会产生约 4000 个小文件，读"近 1 年"反而更慢）。
- **列**：`date, symbol, open, high, low, close, volume, amount, adj_factor`（float32；`adj_factor = kline_daily ÷ kline_daily_raw` 逐日相除，**两分区都已在本地**，无需新数据源）。
- **对齐**：缺数据留 NaN ⇒ **天然承载"数据不足"三态**（新上市 / 退市 / 停牌各就各位）。
- **更新**：`⚡ 增量补齐`（追加新交易日）/ `🔄 从日线重建`（全量）。
- **登记（§11.7 三处 + 一处新）**：`market_db.zones` + `data_manager.ZONE_ORDER/ZONE_LABELS` + **「派生缓存」分组**；**不进 `SYNCABLE`**（不可联网）。
- ⚠ 面板**不复制原始数据**（可删可重建）⇒ 数据主权与空间可控。

**D2 · 计算内核 —— `core/cross_section.py`（纯函数 · 零 UI · 零网络）**
1. **粗筛漏斗**（便宜列，见 D5）→ 候选集；
2. 对候选**一次编译、批量求值**（沿标的轴向量化；面板是二维数组后指标一次算完全部标的）；
3. 产出 **`(dates, symbols)` 三态矩阵** + 可选"关键变量快照"；
4. **M2** = 取指定 `date` 一行；**M3** = 沿 `date` 轴求和（除以**有效样本数**得到占比）。

**D3 · 缓存 —— `data/scan_store.py`**
- **L1（本轮）**：面板读写。
- **L2（后置，先留位）**：键 = `公式指纹 + 复权口径 + 统计范围 + 日期范围 + **数据版本**`。
- ★ **失效必须含"数据版本"**（分区 mtime / `get_latest_date` / 行数指纹）—— **不能只看公式哈希**，
  否则"新下载了数据但不重算" = **静默陈旧**（违反 §5 / §10 铁律）。

**D4 · 线程 —— `ui/workers.py` + `CrossSectionWorker`**
分块（如每 500 只）+ 进度 + 取消 + **竞态守卫**（§9-O5：携带任务标识，过期结果丢弃）。
**UI 不卡**；线程仍只在 `ui/workers.py` 定义（§9-O2）。
...
2. **反向预设**：`bulk_download` 新增 **「全市场扫描就绪」**（全A + 起点 + 复权），一键备齐底座。
3. **面板按方案 B 呈现**：左栏**分组**为「行情源分区」/「派生缓存」；`kline_panel` 在派生组，
   明细/勾选/删除**同一套**，底部动作**按类型切换**（行情源 → 更新到最新 / 重新全量下载；
   派生 → **从日线重建 / 增量补齐 / 释放空间**）；危险动作隔离 + 二次确认（§10-10）。
4. **增量语义共用**：两页**同用** `fresh_within_days = 0`（末日==今天才算最新）、"空增量 = 已最新不是失败"
   ⇒ **杜绝"管理页说最新、扫描页说缺数据"**。
5. **下载顺序由"扫描损失"驱动**：缺口清单带优先级（按成交额降序）⇒ 广度先接近真实。
6. **问题清单回流**：读不出 / 校验不过的标的**回写数据管理的问题清单** + 「重新全量下载该只」入口，
...
1. **`market_db.zones` / `data_manager.ZONE_ORDER·ZONE_LABELS`** 要加派生层 ⇒ 三处登记漏一处就"看不见/报未注册"（§11.7）。
2. **`SYNCABLE` 不含面板** ⇒ 数据管理页的按钮必须**按类型切换**，否则会出现"给派生数据点『更新到最新』"的死按钮。
3. **`backtest_module.py`** 只换两个 `_ComingSoonPage`；**新能力一律落新文件**（`ui/views/scan_view.py` / `breadth_view.py` / `ui/widgets/scan_*` / `breadth_pane.py` / `core/cross_section.py` / `data/scan_store.py`）。
4. **`ui/workers.py`** 新增 `CrossSectionWorker`（线程仍只此一处定义）。
5. **冒烟脚本**：新页面要进 `smoke_pages_overlay` 的迁移护栏与页面断言；新写 `~/.jian_data/*` 要进防污染自检。

#### G. 实施步骤（每步独立可验收 · 一 commit 一步）

- **STEP 0**（零业务代码）：**4 个探针实测** —— ① 读 200 只 parquet（全列 vs 只读列 vs 只读尾部窗口）；② 面板构建耗时 + 内存（float32，10 年）；③ 同一指标「逐只 pandas」vs「面板向量化」耗时对比；④ 端到端一次全市场 M2 / 一年 M3。**探针跑完即删**，数字回填本主案（含标杆：冷启动 ≤ ?s / 热态 M2 ≤ ?s / M3 ≤ ?s）。
- **STEP 1**：`core/cross_section.py` + 单测（合成数据：三态正确 · 粗筛生效 · 命中矩阵逐位可验）。
- **STEP 2**：`kline_panel` 分区 + `增量补齐`/`从日线重建` + 三处登记 + 数据管理页**按方案 B 分组呈现**（含危险动作隔离）。
- **STEP 3**：`data/scan_store.py`（L1 + 预留 L2）。
- **STEP 4**：`CrossSectionWorker`（进度 / 取消 / 竞态）。
- **STEP 5**：**M2 页**（先「自选 / 指数成分」小范围跑通 → 放开全A）+ 实测重复扫描耗时（决定 L2）。
- **STEP 6**：**M3 页**（复用 L1/L2 + 指数副图 x 联动 + 增量到最新 + 全量重算闸门）。
- **STEP 7**：联动收尾（就绪度体检 / 缺口补齐 / 问题清单回流 / 防污染自检）。
...
| **复权因子缺失区间**（两分区不完全对齐） | 该段**显式标注"不支持复权"**，**不猜、不插值** |
| **全市场 IO 把 UI 卡死** | 分块 + QThread + 进度 + 取消；面板把 IO 前置化 |
| **内存峰值**（5400 × 4000 × 9 列 ≈ 数百 MB） | 按年分块载入 + 及时释放；必要时按标的块分批 |
| **两页口径分裂** | **就绪度模型只有一份**；下载一律走 `MarketSyncService`（§9-H） |

#### I. 本主案明确"不做"（防范围失控）

- ❌ **估值 / 财务类筛选**（PE/PB/ROE…）—— `valuation` / `fin_report` **无数据源**，属独立的数据源扩建。
- ❌ **真·换手率**（需流通股本）—— 本轮用**相对量**替代；"接股本数据源"登记为后续项。
...
`kline_panel` 三处登记齐全 + `SYNCABLE` 不含它。
...
| 版本 | 一句话 |
|---|---|
| **1.27** | 文档分层重构（本文件瘦身 + `docs/` 三册）· 零业务代码 |
| **1.26** | 复盘页版式收口 §9-U + `review`/`data_feed` 剩余拆分 §9-L |
| **1.25** | 画线工具重做（点选绘制 / 通道语义 / 交互修复）· 32 种全通 |
...
- **A. ✅ 已修（v5.7）版本号滞后；🔁 编号体系于 v6.16 改版（用户拍板）**：
  ...
  · **当前值 = `1.26`**（= 复盘页版式收口 §9-U + `review` / `data_feed` 拆分 §9-L；1.25 = 画线
    工具重做「点选绘制」+ 通道语义修正；1.24 = 侧边栏重设计 + 自选分组；1.23 = 行情工作台收口）；
...
> **v6.31（§7-B1/B2 立项：M2 全市场横截面筛选 / M3 广度统计 · 设计定稿，待按 STEP 施工）**
> 用户 2026-09-19 看完 `design/1.27-scan-breadth-ui/`（方案 README + 两个可点原型）后拍板 6 条，
> 详见 **§7-B1/B2 主案规格 A 节**。**摘要**：
>   ① **主轴** = 派生扫描面板 `kline_panel`（日期×标的宽表，**按年分区**）+ **两段式漏斗**（先粗筛后精算）
>      —— 不做"逐只读文件直接算"（湖是按标的存的，看一列要开 5000+ 个文件）；
>   ② **复权一次到位**：面板存「不复权 + 复权因子」，**指标按用户需求复权**（不复权/前复权/后复权）；
>   ③ **面板可见性 = 方案 B**：数据管理页左栏**分两组**（行情源 / 派生缓存），动作按类型切换
>      （派生 → 从日线重建 / 增量补齐 / 释放空间）；
>   ④ **缓存 L1 先、L2 后置**（重复扫描 > 2s 才做 L2）；
>   ⑤ **粗筛多阈值 + 内置默认 + 全可调**；**真·换手率需股本数据源 ⇒ 本轮用「相对量」替代**；
>   ⑥ **三态铁律**：命中 / 未命中 / **数据不足** —— 绝不用 0/False 冒充，广度必须显示"有效样本数"。
> **一 commit 一步**：STEP 0 探针实测（零业务代码）→ 1 内核 → 2 面板+数据管理分组 → 3 scan_store
> → 4 Worker → 5 M2 页 → 6 M3 页 → 7 联动收尾 → 8 文档与发版。
> ✅ 本批**零业务代码变更**（只新增 `design/1.27-scan-breadth-ui/` 与本文档主案）。
...
- [ ] **改了公式引擎 / 图表渲染 / 图层公共件 / 控件样式（含 `SegmentedControl`）/ **工具行 chips 规则（`chip_mru`）** / 标注模型 / 配方库 / 周期重采样（含**分钟档位**）/ 自选股 / 复权口径 / 图元拖动 / 坐标轴 / 图表宿主读数条（§7-B6）/ 回测成交口径（§7-B5）/ **数据源护栏（§9-V：非正价拦下 · 兜底源单位统一）** / **K 线图元画法（§9-V-3：一字板横档）** / **画线类型规格表 / 附属图元 / 图元小件 / 绘制会话 / 画线填充配色（`chart_style.annotation_fill`）**（即 `annotation_shapes` / `annotation_decos` / `annotation_items` / `annotation_draw_session` / `chart_style` 任一文件），跑过 `py tests/smoke_chart.py` 吗？**（**589 项**，纯组件、离屏）
- [ ] **改了行情工作台页面（`trading_desk.py`）/ `ui/widgets/desk_*.py` 任一模块 / 回测页「成交模型」行 / 标注交互层 / **画线类型目录（新增类型、`implemented` 翻牌）**？** → 跑 `py tests/smoke_pages_overlay.py`（**396 项**，含 **§7-B6 的「迁移护栏」+ 顶栏分段控件/分钟档位 + 工具行 chips + 图标轨/分页面板/折起（含**富余宽度归图表、折起后左侧只剩图标轨**两条不变量）+ 读数条 + **口径回执的"除权跳空定位 / 数据体检"**+ STEP 6 的"实现落在哪个 `desk_*.py`"**：公共面被改名、旧入口（`cb_period`/`cb_adjust`/`cmb_tool`）被复活、**把薄壳写成空函数**、**分栏比例退化**、**回执退回"不解释"**，都会立刻红）；
...
- [ ] **改了复盘页（`ui/views/review.py`）/ `ui/widgets/review_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**396 项**，含 **「复盘页迁移护栏」+ §9-U 分栏不变量**：
...
- [ ] **改了回测页/工作台/复盘页的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**396 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；三条离屏打桩见 §11.5-20，**别删**）
...
- [ ] **动了数据源/落盘护栏吗？（`data/akshare_feed.py` 的 `drop_unusable_price_rows` /
      ...
      （**589 项**：非正价行必须被拦下、兜底源成交量必须 ×100 成「股」、量纲接缝判据不许误报
