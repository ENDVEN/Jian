...
| **S2-7** | 关于与更新 | **✅ 已落地**：版本（动态现算）· **检查更新**（手动查**必有回执**："已是最新 1.57" 也算答案）· 打开发布页 · 反馈页 · **复制诊断信息**（版本/平台/自检报告，**零请求、无凭据值**）· 打开日志目录 | 注册表 6 项 · `ui/workers.UpdateCheckWorker` · 复用 `core/updater` 版本口径 | ✅ |
| **S2-8** | 收口与文档 | **✅ 已落地**：**收口判据机器化** —— ① **没有「空组且无说明」**（暂跳的 appearance/keys 组写明"哪一步接进来"）② 已接内容的组 ≥5 ③ 全项**类型合法 + 落点齐全**（无半成品项）④ 设置中心**只有一个编辑面**（旧对话框 = 无入口兼容壳）；文档/`HISTORY`/快照对齐。**下一步 = §7-B14 第一次大扫除盘点**（触发判据已写死）| 文档 + 台账 | ✅ |
