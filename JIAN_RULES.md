...
| 项 | 值 |
|---|---|
| APP 版本 | **1.48**（三处同步 §9-A：`APP_VERSION` ✅ / `version.json` ✅ / commit 首词 `1.48` ✅；**push 待用户**） |
| 文档版本 | **v6.66**（不复权分区的**下载入口**：批量口径选择 + 空分区回落） |
| 最近三版 | `1.48` 不复权分区下载入口 · `1.47` 复权自愈与字体版权收口 · `1.46` M2/M3 增强与运行历史重做 |
| **当前主线** | **v6.66 不复权分区的下载入口**（`1.48` **本地已提交**，**待用户实测 + push（未推送）**）—— ① 批量预下载加**口径选择**（前复权 / 不复权 / 两个都下）；② 数据管理页**空分区也能下载新标的**（回落过滤框代码）。**下一步候选** = §7-B4 回测进阶 / 深色主题 / desk 一键「修回历史」。 |
| **断点** | **v6.66 复权"能用但用不上"**（用户 2026-09-25 实测："切不复权还是没数据；数据管理里 raw 只有 1 项"）。**探针先排除嫌疑**：数据层与取数**都正常**（`refresh_one(raw)` 真能下 1665 行；行情页切不复权**真线程端到端**也成功）⇒ 真因是**入口**：批量预下载**无口径选择**（永远只写 `kline_daily`）+ 数据管理对**空分区无入口**（没行可勾 ⇒ 按钮恒灰）。**修法**：批量加口径（"两个都下"＝两个任务）+ `data_manager._sync_targets()` 空分区回落"过滤框里的代码"（**删除仍只认勾选**）+ 空分区把出路写进状态行。顺手抓到一个真坑：`str.isalnum()` **对中文为 True** ⇒ 加 `isascii()`。坑 = §11.5-92。验收两份冒烟全绿（**数字唯一出处 §11.7**）。 |
...
- **业务文件 ≥400 行（★v1.46 现测 = 本轮动过的文件；其余仍是 v1.45 的数。涨落史不抄在这里，
  ...
  ⚠ `ui/widgets/desk_data.py` **517**（★v1.47：价格接缝体检 `_price_seam` +失败回执退出"正在取数"；
    ★v6.66：同步 tooltip/回执带口径；再长把"体检判据"整段抽成 `ui/widgets/desk_health.py`）>
   ...
   **用 `git worktree add ..\Jian-baseline HEAD` 拉一份干净基线跑→ 同样 816/1** ⇒ 环境/时机敏感的
   ...
   - **11.5-93** 【v6.66 · 发布物】**`version.json` 是"老用户的更新清单"，改它必须过一遍 `json.load`**：
     ...
     **口径**：① 写完立刻 `json.load` 验一次；② 现有**机器护栏**盯着"合法 JSON / version == `APP_VERSION` /
     url 指向 Releases / notes 非空"（§11.7「三处同步」的机器版）；③ JSON 里的引号用全角「」或转义，别裸写。



### 11.6 当前"下一步做什么"的推荐顺序（历史刷新**倒序**排列：主清单之下**第一块就是最新**）

> ✅ **已落地（v6.66 / `1.48` · 不复权分区的**下载入口** · 2026-09-25 用户实测驱动）**
...
> ⑤ **顺手堵住一个发布物事故（本批自查抓到）**：手工回写 `version.json` 时 notes 里写了**裸双引号** ⇒
>   整份文件成了**非法 JSON**（编辑器不报错、只有 `json.load` 会炸）—— 而它是**老用户的更新清单**
>   （`core/updater.py` 每次启动比对）⇒ 解析失败 = **所有老用户的更新检查崩**。已修 + 加**机器护栏**
>   （`smoke_pages_overlay` 末段：合法 JSON / version == `APP_VERSION` / url 指向 Releases / notes 非空）。
>   坑 = **§11.5-93**。
> ⑥ **"抓的是哪一份"做成看得见**（用户再问："切了口径点云端同步，抓的不是我要的那份？"）：
>   ① 行情页「☁️ 云端同步」tooltip **实时**写明"本次同步目标 = 当前口径 + 分区"（切换即变），
>   同步中回执 `正在同步 不复权（kline_daily_raw）…`、完成回执 `已更新，新增 N 行（不复权）`；
>   ② M1/M2/M3 的「更新到最新」tooltip 与回执写明 **口径固定 = 前复权日线**（单一出口
>   `readiness_flow.SYNC_CALIBER_LABEL`）；回测页补数据回执改"本地**前复权**日线未到…"。
>   ⚠ 关键认知：**回测/扫描口径 ≠ 行情页图表口径**（前者固定前复权，后者随你切），以前界面从没说，
>   用户自然会以为"切了不复权，回测也跟着换口径"。§10-10 同族。
> 断言 `smoke_pages_overlay` **686 → 708**（+13 入口 +4 发布物 +5 口径可见性），`smoke_chart` **864** 不变。
...
- [ ] **改了行情工作台页面（`trading_desk.py`）/ `ui/widgets/desk_*.py` 任一模块 / 回测页「成交模型」行 / 标注交互层 / **画线类型目录（新增类型、`implemented` 翻牌）**？** → 跑 `py tests/smoke_pages_overlay.py`（**708 项**，含 **§7-B6 的「迁移护栏」+ 顶栏分段控件/分钟档位 + 工具行 chips + 图标轨/分页面板/折起（含**富余宽度归图表、折起后左侧只剩图标轨**两条不变量）+ 读数条 + **口径回执的"除权跳空定位 / 数据体检"**+ STEP 6 的"实现落在哪个 `desk_*.py`"**：公共面被改名、旧入口（`cb_period`/`cb_adjust`/`cmb_tool`）被复活、**把薄壳写成空函数**、**分栏比例退化**、**回执退回"不解释"**，都会立刻红）；
...
- [ ] **改了复盘页（`ui/views/review.py`）/ `ui/widgets/review_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**708 项**，含 **「复盘页迁移护栏」+ §9-U 分栏不变量**：
...
- [ ] **改了全市场筛选页（`ui/views/scan_view.py`）/ `ui/widgets/scan_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**708 项**，含 **「M2 公共面护栏」+ 版式与口径不变量**）。
...
- [ ] **改了广度统计页（`ui/views/breadth_view.py`）/ `ui/widgets/breadth_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**708 项**，含 **「M3 公共面护栏」+ 双窗格/区间/增量不变量**）。
...
- [ ] **改了回测页/工作台/复盘页的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**708 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；四条离屏打桩见 §11.5-20 与 §11.5-79（★第④条 v1.42 补：**异步回测与后台下载也不许写真目录** —— 看到"只有几条真实数据类断言红"先问“有没有东西正在写”），**别删**）
...
- [ ] **改了后台下载（`ui/download_hub.py` / `ui/widgets/download_bar.py` / `download_queue_panel.py`）或四个下载入口（`bulk_download` / `data_manager` / `readiness_flow` / `main_window.downloads`）吗？** → 跑 `py tests/smoke_pages_overlay.py`（**708 项**，含「§7-B11 后台下载」段）。⚠ 四条红线：
