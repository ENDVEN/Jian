...
| 项 | 值 |
|---|---|
| APP 版本 | **1.32**（= 本次提交首词 · **三处已同批同步 §9-A**：commit 首词 / `APP_VERSION` / `version.json`；**push 待用户**） |
| 文档版本 | **v6.41**（修复成分股名称列全空 + 内核警告 UI 出口，§11.5-63） |
| 最近三版 | `1.32` 成分股名称列修复 · `1.31` 成分股代码格式修复 · `1.30` STEP 6 联动收尾 |
| **当前主线** | **§7-B1/B2 —— M2 全市场横截面筛选 / M3 广度统计**（**STEP 0–6 ✅**：内核 / 缓存 / 线程 / M2 页 / M3 页 / **联动收尾**全部就位；剩余 = STEP 7 可选性能档（**暂不排期**）+ STEP 8 发版收尾） |
| **断点** | **STEP 6 ✅**：交付 = `data/readiness.py`（footer 只读体检，实测 0.66 ms/只）+ `ReadinessWorker`（第 8 个 Worker）+ `ReadinessFlow`（M2/M3 共用控制器：体检→缺口可见→⬇补齐→复检，回调**绑定各自 scope** 防穿网）+ 成分股失败改 `friendly_constituent_message` 人话诊断 + bulk_download「⚡全市场扫描就绪」预设；**STEP 6 前置复核已做**：真机 368 只冷扫 2.89 s（7.85 ms/只，与 B3 复核一致）⇒ 全市场外推 ≈42 s，**决策 = 接受引擎路径性能**（向量化档只在用户备齐全 A 后按需评估）；成分股链路**真机联网实测通**（000300→288 只 / 000905→429 只，8~14 s，界面已注明耗时预期）；`smoke_chart` **695 项**、`smoke_pages_overlay` **457 项**全绿；**v6.40 追加**：用户实测 M2/M3 选沪深300 全部失败 —— 根因 = 两套指数代码约定碰撞（M2/M3 复用 `INDEX_PRESETS` 带前缀键 `sh000300`，成分股三接口只要 6 位裸码），已修（`normalize_cons_code` 收在行情源边界 + 格式不对快速失败），真机复验 `sh000300`→288 只 / `sh000016`→50 只拉通（详见 §11.5-62）。**v6.41 追加**：用户实测名称列全空 —— 成分股异步回包漏装配 `_names`（自选/全A 分支都有、唯独它没有）⇒ 名称列空 + **「剔除 ST」整轮静默失效**（内核出过警告但 UI 吞了）；已修（回包补装配 + 两页 `_on_finished` 给警告出口 + readiness 回执守卫合并"已有扫描结果"条件），真机复验名称正常；详见 §11.5-63。`smoke_chart` **695** / `smoke_pages_overlay` **461** 全绿；**下一步 = 用户手动实测（小范围→全A）+ STEP 8 文档/发版收尾**。⚠ **并行会话警告**：检测到 Qoder 正在并行施工 §7-B8 R7（`desk_formula.py`/`desk_layout.py` 的未提交半成品 + `tests/test_r7_sort.py`），本轮提交**不含**这三个文件 |
...
| 📐 市场回测 | `BacktestModule` | [~] | M1 单股回测完整（含 **📚 配方库 / 💾 存为配方 / 📤 送到行情页**，v6.11/P7）；M2 全市场筛选完整（v6.37/STEP 4）；M3 广度统计完整（**v6.38/STEP 5**：**📊 广度折线（家数/占比%）+ 指数副图 x 联动** + **⚡增量到最新**（尾段续接，历史一天不动）+ **⟳全量重算**二次确认 + 区间切片零成本；见 §7-B1/B2 主案 **B2 / B3 / D7**） |
...
│                        #       （§7-B6 STEP 3b/3c/4）+ 成交模型行/教学弹窗/导出口径**页面级**验收（**457 项**）
...
- **业务文件 ≥400 行（1.26 收官实测降序）**：
  ...
  ⚠ `ui/widgets/breadth_flow.py` **402**（v6.39 +成分股诊断/体检接线又越线；**登记不返工** ——
    收口时可把"范围解析→体检接线"这对重复动作与 `scan_flow` 一起收进 `ReadinessFlow`）。
  ...
  （按 v6.16 口径不再标数字）。
- **验收脚本不参与"业务文件瘦身"**：`tests/smoke_chart.py 3193`、
  `tests/smoke_pages_overlay.py 2373`（总行数；**它们不是业务文件，别为了让数字好看去拆**）。
...
| **全市场筛选 M2** | 某一天的全市场横截面筛选：三态 KPI + 结果表（双击跳行情）· 粗筛漏斗（含换手率/市值）· 切日期零成本 · **就绪度体检 + ⬇补齐缺失**（v6.39） | `ui/views/scan_view.py` + `ui/widgets/scan_*` + `readiness_flow`（v6.37/39） |
| **广度统计 M3** | 逐日"有多少只满足条件"：广度折线（家数/占比% + MA5 平滑）+ 指数副图 x 联动 · **⚡增量到最新**（尾段续接，历史一天不动）· **⟳全量重算**（二次确认）· 区间切片零成本 · **就绪度体检 + ⬇补齐缺失**（v6.39） | `ui/views/breadth_view.py` + `ui/widgets/breadth_*` + `readiness_flow`（v6.38/39） |
...
- **B8 [ ] ⭐`1.24` 侧边栏重设计（A 方案）+ 现有功能改进（v6.24 立项 · 用户 2026-09-17 拍板方向）**：
  ...
  **附图纵轴对齐** / 附图数量仍由 chips 控制 / 画线类型**图标+数字** / 支持 30+ 类型 / 数据页暂缓）。
  ⚠ 其中 **R7（副图可排序 - 底层 API✅/UI 入口待施工）/ R8（纵轴对齐✅已完成）** 是代码级真实缺陷，已核实；
...
#### B2. STEP 0 实测（2026-09-19 · 探针跑完即删 · **数字回填在此**）

> ✅ **本节 3 条改写已于 2026-09-19 获用户确认**，并已落到 **D1 / D2 / D3 / D5 / D6 / D8**；
> 本节保留"实测数字 + 当时的推理"作依据，**结论以 D 节为准**。
>
> 环境：本机 `kline_daily` = **368 只 / 41.5 MB / 90.1 万行 / 4060 个全局交易日**。
> 下表"全市场"列 = **按标的数线性外推 ×14.7**，仅作量级参考；真机全市场规模须在 STEP 5 前置复核。
...
**改写的三句话**（依据 = 上表；**落地与结论以 D 节为准**；逐版叙事见 `docs/JIAN_HISTORY.md` v6.33）

① **IO 不是瓶颈**（P1a/b/c）⇒ "落盘面板"的首要理由不成立 → **会话内存**（D1 / D3）；
② **真换手率 / 流通股本本来就有**（P1b 意外发现，覆盖 362/368）→ **可用**（D5）；
③ **复权只能单口径**（`kline_daily_raw` 只有 1 只）→ 本轮**只上前复权 + 界面明示**（D8）。

> 两条数据健康问题（**分区列结构不一致** / **`shift` 跨空洞语义**）**已各自成节**：
> 见 **§9.1 / §9.2**，此处不重复；**完整推理原文**见 `docs/JIAN_ARCHIVE.md`「五、STEP 0 探针明细」。

#### B3. P4 · 端到端标杆（2026-09-19 · **STEP 0 收尾**）

实测环境同 B2（本机 368 只 / 90.1 万行 / 最新交易日 **2026-09-18**）。
...
1. **⚠ 精算单只耗时比 P3 高 2.4×（3.90 vs 1.60 ms/只）** —— 差异**不在引擎**，而在 P4 里我用
   **逐行 Python 累加逐日家数**（`for d, v in zip(g["date"], sig)` ≈ 90 万次迭代）。
   ⇒ **STEP 1 必做优化**：改"日期 → 整数索引 + `np.bincount` 累加"（M3 家数）+ "目标日直接索引"（M2 截面），
   预计精算回到 **~1.6 ms/只 ⇒ 全市场 ~8.6 s、端到端 ~14 s**。
2. **⚠ "漏斗能让精算量降一个数量级"在本例不成立** —— 实测只砍掉 **14/368（3.8%）**
   （有效数据不足 2 + 价格/量/额不符 12）。原因：`CROSS + MA20` 这类**宽条件**通过率本来就高。
   ⇒ 漏斗仍是**廉价保险**（37 ms，且负责"数据不足"的正确归因），但**不能指望它救性能**；
   真正的性能杠杆是 **① 引擎逐只够不够快 ② 逐日累加是否向量化**。
   （`design/1.27-scan-breadth-ui/README.md` 里那句"降一个数量级"**已按实测修正**。）

**建议验收标杆（待真机 5400 只复核 → 放 STEP 4 前置）**

- **全市场首次扫描 ≤ 20 s**（实测外推 26 s 含未优化累加；优化后预计 ~14 s，留余量）；
- **缓存命中后切日期 / 切粗筛 ≤ 50 ms**（实测 0.03 ms，余量极大）；
- **三态必须同时显示有效样本数**（本例 354/368）。

**STEP 1 复核（v6.34 · 内核已能端到端跑）**：真实行情 368 只 **2.60 s（7.06 ms/只）** ⇒ 全市场外推
**≈38 s**，**高于上表 ≤20 s 的标杆**。差额**不在引擎**（P3 实测 1.60 ms/只），而在 **per-symbol 的固定开销**
（`prepare_frame` 的 copy/sort/5 次 `to_numeric`、逐行掩码、逐标的循环本身）—— P4 的 26 s 只算了
**354 只候选**且不含掩码，两者不可直接比。
⇒ **记为 STEP 4 的性能对账项**：① 读数时就保证 dtype 与序，省掉 `prepare_frame` 的拷贝；
② 合并掩码的中间数组；③ **先按基准日粗筛再求值**（M2 可省，M3 需要全序列 ⇒ 省不掉）。
**标杆先不放松**：等 STEP 4 实测后再定"接受 38 s / 还是启用向量化档（STEP 7）"。
...
| **1.31** | 修复 M2/M3 指数成分股代码格式（`normalize_cons_code` 边界规范化，§11.5-62） |
...
> **v6.38（§7-B1/B2 STEP 5 落地：M3 广度统计页 · 1.29）**
> 交付：`ui/views/breadth_view.py` + `breadth_layout|flow|result|chart`（全部 <400 线）+
> `scan_cached(incremental=True)`（`find_base` 同配置旧条目 + `merge_tail` 尾段续接）+
> `CrossSectionWorker` 增量通道 + 偏好键 `breadth_ui`；`_ComingSoonPage` 占位类退役。
> 交互 = 广度折线（家数/占比% + MA5 平滑）+ 指数副图 x 联动（缺数据后台补拉一次）+
> 区间六档纯切片 + **⚡增量到最新**（历史矩阵一天不动；数据没变回执"已算到最新"）+
> **⟳全量重算**（二次确认）。验收：`smoke_chart` **678** / `smoke_pages_overlay` **447** 全绿 +
> 全仓 compileall。⚠ 如实登记：引擎逐只路径下"增量"对小范围秒级、对全市场 ≈ 全量重扫
> （per-symbol 固定开销主导，B3 复核）；真秒级等 STEP 7 宽表档。
> **下一步 = STEP 6 联动收尾**：① 前置复核 **真机全 A 重复扫描耗时**（外推 ≈38 s vs 标杆 ≤20 s，
> B3 的 STEP 1 复核）⇒ 决定"接受 / 优化 prepare_frame / 启用 STEP 7"；② 就绪度体检（双向唯一口径）；
> ③ `bulk_download`「全市场扫描就绪」预设；④ 缺口按成交额优先；⑤ 问题清单回流数据管理页；
> ⑥ 防污染自检。然后 STEP 7 可选性能档 → STEP 8 文档回写 + 发版。
...
> **一 commit 一步（v6.33 已按 STEP 0 实测重排）**：STEP 0 探针 ✅（P1/P2/P3 完成 · P4 收尾）
> → 1 内核 → 2 会话内存缓存 → 3 Worker → 4 M2 页 → 5 M3 页 → 6 联动收尾
> → 7 可选性能档（宽表向量化）→ 8 文档与发版。
> ✅ 本批**零业务代码变更**（只新增 `design/1.27-scan-breadth-ui/` 与本文档主案）。
> ⚠ **立项时的 3 项拍板已被 STEP 0 实测改写**（落盘面板 → **会话内存** · 换手率**可用** ·
> 本轮**前复权单口径**）—— **以主案 D 节为准**，依据见 **B2** 与 **§9.1 / §9.2**。
...
- [ ] **改了复盘页（`ui/views/review.py`）/ `ui/widgets/review_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**457 项**，含 **「复盘页迁移护栏」+ §9-U 分栏不变量**：
...
- [ ] **改了全市场筛选页（`ui/views/scan_view.py`）/ `ui/widgets/scan_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**457 项**，含 **「M2 公共面护栏」+ 版式与口径不变量**）。
      ⚠ 四条最容易顺手改坏的：① **常驻行必须 ≤3**（粗筛阈值收在抽屉里，别往结果区上方加行）；
...
- [ ] **改了广度统计页（`ui/views/breadth_view.py`）/ `ui/widgets/breadth_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**457 项**，含 **「M3 公共面护栏」+ 双窗格/区间/增量不变量**）。
      ⚠ 五条最容易顺手改坏的：① **常驻行必须 ≤3**（⚡/⟳ 收在摘要条，别往结果区上方加行）；
...
- [ ] **改了回测页/工作台/复盘页的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**457 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；三条离屏打桩见 §11.5-20，**别删**）
...
- [ ] **动了数据源/落盘护栏吗？（`data/akshare_feed.py` 的 `drop_unusable_price_rows` /
      ...
      （**692 项**：非正价行必须被拦下、兜底源成交量必须 ×100 成「股」、量纲接缝判据不许误报
