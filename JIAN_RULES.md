...
| 项 | 值 |
|---|---|
| APP 版本 | **1.33**（= 本次提交首词 · **三处已同批同步 §9-A**：commit 首词 / `APP_VERSION` / `version.json`） |
| 文档版本 | **v6.43**（M2/M3 体验修复轮：图表可读性 / 先选后扫 / 成分股换权威源 / 缺数据诚实化，§11.5-64…69） |
| 最近三版 | `1.33` M2/M3 体验修复轮 · `1.32` 成分股名称列修复 · `1.31` 成分股代码格式修复 |
| **当前主线** | **§7-B1/B2 —— M2 全市场横截面筛选 / M3 广度统计**（**STEP 0–6 ✅ + 体验修复轮 ✅**：内核 / 缓存 / 线程 / M2 页 / M3 页 / 联动收尾 / **可读性与诚实化**全部就位；剩余 = STEP 7 可选性能档（**暂不排期**）+ STEP 8 发版收尾） |
| **断点** | **v6.43（1.33）M2/M3 体验修复轮 ✅**（用户 2026-09-20/21 两轮实测驱动；全量叙事 = `docs/JIAN_HISTORY.md` v6.43 行，坑 = §11.5-64…69）：① **M3 图表可读性根治** —— `breadth_chart` 改回 **bar 序号轴**（旧版喂 epoch 秒给 `attach_all`，y 跟随 100% 静默失效，§11.5-64）+ 指数按日对齐只画当前区间 + 柱状/折线双视觉（默认柱状+MA5，家数刻度取整）+ 指数四图形（折线/面积/K线/美国线 `OhlcBarItem`，缺开高低退折线）+ **分界把手可见可拖**（`ChartHost.enable_divider_drag`，按下锁定参考系⇒双向跟手，§11.5-67）+ 两窗格轴对齐（R8 第二版固定左槽，不留白）。② **M2 先选后扫** —— 基准日日历自由选（默认=本地最新交易日，体检校准）；内核轴外日子**就近落位+出声**；◀▶/日历双向同步。③ **缺数据全链路诚实化** —— 无文件标的逐只记「数据不足」进总数（旧版静默消失，§11.5-65）；读数为空也不回空白；扫描前闸门二次确认；体检占位文案不再冻结（§11.5-66）。④ **成分股换中证官网权威源**（同花顺 300→288/500→429 缺斤短两）+ 快照日期/名义只数核对上界面 + 修「指数代码」选列回归（§11.5-68）；幸存者偏差已界面声明，根治路登记 **§7-D 类 D4 时点成分股**。⑤ 表格性能三件套（§11.5-69）+ 2000 行截断说实话。验收：`smoke_chart` **707**（连跑两轮全绿；含 vline 拖拽断言 1px flaky 根治：抓取点改取 viewbox 中线）/ `smoke_pages_overlay` **482** 全绿 + compileall；联网实测沪深300 → **300 只、不含 000300、快照 2026-09-18**。⚠ §7-B8 R7 并行半成品已移入 stash（desk_formula/desk_layout），本轮不含。**下一步 = 用户实测手感（分界线/闸门/四图形）→ STEP 8 发版收尾**；§9.3 代理文案待修清单仍挂（需用户拍板） |
...
│                        #       （§7-B6 STEP 3b/3c/4）+ 成交模型行/教学弹窗/导出口径**页面级**验收（**482 项**）
...
- [ ] **改了公式引擎 / 图表渲染 / 图层公共件 / 控件样式（含 `SegmentedControl`）/ **工具行 chips 规则（`chip_mru`）** / 标注模型 / 配方库 / 周期重采样（含**分钟档位**）/ 自选股 / 复权口径 / 图元拖动 / 坐标轴 / 图表宿主读数条（§7-B6）/ 回测成交口径（§7-B5）/ **数据源护栏（§9-V：非正价拦下 · 兜底源单位统一）** / **K 线图元画法（§9-V-3：一字板横档）** / **画线类型规格表 / 附属图元 / 图元小件 / 绘制会话 / 画线填充配色（`chart_style.annotation_fill`）**（即 `annotation_shapes` / `annotation_decos` / `annotation_items` / `annotation_draw_session` / `chart_style` 任一文件），跑过 `py tests/smoke_chart.py` 吗？**（**707 项**，纯组件、离屏）
- [ ] **改了行情工作台页面（`trading_desk.py`）/ `ui/widgets/desk_*.py` 任一模块 / 回测页「成交模型」行 / 标注交互层 / **画线类型目录（新增类型、`implemented` 翻牌）**？** → 跑 `py tests/smoke_pages_overlay.py`（**482 项**，含 **§7-B6 的「迁移护栏」+ 顶栏分段控件/分钟档位 + 工具行 chips + 图标轨/分页面板/折起（含**富余宽度归图表、折起后左侧只剩图标轨**两条不变量）+ 读数条 + **口径回执的"除权跳空定位 / 数据体检"**+ STEP 6 的"实现落在哪个 `desk_*.py`"**：公共面被改名、旧入口（`cb_period`/`cb_adjust`/`cmb_tool`）被复活、**把薄壳写成空函数**、**分栏比例退化**、**回执退回"不解释"**，都会立刻红）；
...
- [ ] **改了复盘页（`ui/views/review.py`）/ `ui/widgets/review_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**482 项**，含 **「复盘页迁移护栏」+ §9-U 分栏不变量**：
...
- [ ] **改了全市场筛选页（`ui/views/scan_view.py`）/ `ui/widgets/scan_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**482 项**，含 **「M2 公共面护栏」+ 版式与口径不变量**）。
...
- [ ] **改了广度统计页（`ui/views/breadth_view.py`）/ `ui/widgets/breadth_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**482 项**，含 **「M3 公共面护栏」+ 双窗格/区间/增量不变量**）。
...
- [ ] **改了回测页/工作台/复盘页的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**482 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；三条离屏打桩见 §11.5-20，**别删**）
...
- [ ] **动了数据源/落盘护栏吗？（`data/akshare_feed.py` 的 `drop_unusable_price_rows` /
      ...
      （**707 项**：非正价行必须被拦下、兜底源成交量必须 ×100 成「股」、量纲接缝判据不许误报
