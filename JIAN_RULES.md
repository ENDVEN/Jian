...
│   └── smoke_pages_overlay.py # 1082行 回测页/工作台叠层 + 标注 + 互送 + 自选/周期(含**分钟**)/
│                        #       复权/坐标轴 + 顶栏分段控件/chips + 图标轨/分页面板/折起
│                        #       （§7-B6 STEP 3b/3c/4）+ 成交模型行/教学弹窗/导出口径**页面级**验收（**279 项**）
│                        #       + **§7-B6 STEP 0「行情工作台迁移护栏」**：公共面改名/删除立刻红
...
- **业务文件 ≥400 行（1.23 收官实测降序）**：
  ⚠ `ui/views/review.py 1067`（**唯一还越线的页面**，与 §9-U/§9-L 一起办）>
  `data/data_feed.py 827` > `ui/views/backtest.py 787`（1.22 拆分收官）>
...
- ⚠ **剩下的体积债在别的页面**：`review.py`（1067，同时是 §9-U 的版式收口对象，**动它时一起办**）
  与 `data/data_feed.py`（827，纯重构）。新能力一律落到 `ui/widgets/*` 或 `data/*_store`（§10-12）。
...
- [x] **代码物理拆分纪律**（v5.13 复核）：
  ...
  - [x] **大文件拆分（1.22 收官）**：`ui/views/backtest.py` 1697 → 1467 → 1156 → **787（-54%）**，
    ...
    `review.py 1067` / `data/data_feed.py 827` 属**下一批**：复盘页与 §9-U 一起办。
...
> ⚠ `trading_desk.py` 已 1375 → **345**（v6.22 拆分收官），此条对它是历史记录）。
...
- **A. ✅ 已修（v5.7）版本号滞后；🔁 编号体系于 v6.16 改版（用户拍板）**：
  ...
  · **当前值 = `1.23`**（= 行情工作台版式收口 §7-B6 + `trading_desk` 拆分）；
...
- **U. 【版式：其他页面未按 §10-14 收口】（1.22 登记 · 长期待办 · 用户 2026-09-16 明确要求
  ...
  | `ui/views/data_manager.py`(473) / `dashboard.py` | 分区清点/同步控件常驻 | 轻症，可顺路 |
...
>   （`review.py 1067` 最大 —— 与 §9-L 剩余拆分**一起办**、改一遍；`data_manager.py 473`）；
>   ② §9-L 剩余拆分（`review.py 1067` / `data/data_feed.py 827`）；③ §7-B1/B2（M2 全市场单日筛选 /
...
- [ ] **改了公式引擎 / 图表渲染 / 图层公共件 / 控件样式（含 `SegmentedControl`）/ **工具行 chips 规则（`chip_mru`）** / 标注模型 / 配方库 / 周期重采样（含**分钟档位**）/ 自选股 / 复权口径 / 图元拖动 / 坐标轴 / 图表宿主读数条（§7-B6）/ 回测成交口径（§7-B5）/ **数据源护栏（§9-V：非正价拦下 · 兜底源单位统一）** / **K 线图元画法（§9-V-3：一字板横档）** / **画线类型规格表 / 附属图元 / 图元小件 / 绘制会话 / 画线填充配色（`chart_style.annotation_fill`）**（即 `annotation_shapes` / `annotation_decos` / `annotation_items` / `annotation_draw_session` / `chart_style` 任一文件），跑过 `py tests/smoke_chart.py` 吗？**（**574 项**，纯组件、离屏）
- [ ] **改了行情工作台页面（`trading_desk.py`）/ `ui/widgets/desk_*.py` 任一模块 / 回测页「成交模型」行 / 标注交互层 / **画线类型目录（新增类型、`implemented` 翻牌）**？** → 跑 `py tests/smoke_pages_overlay.py`（**372 项**，含 **§7-B6 的「迁移护栏」+ 顶栏分段控件/分钟档位 + 工具行 chips + 图标轨/分页面板/折起（含**富余宽度归图表、折起后左侧只剩图标轨**两条不变量）+ 读数条 + **口径回执的"除权跳空定位 / 数据体检"**+ STEP 6 的"实现落在哪个 `desk_*.py`"**：公共面被改名、旧入口（`cb_period`/`cb_adjust`/`cmb_tool`）被复活、**把薄壳写成空函数**、**分栏比例退化**、**回执退回"不解释"**，都会立刻红）；
...
- [ ] **改了回测页/工作台的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**372 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；三条离屏打桩见 §11.5-20，**别删**）
...
- [ ] **动了数据源/落盘护栏吗？（`data/akshare_feed.py` 的 `drop_unusable_price_rows` /
      ...
      （**574 项**：非正价行必须被拦下、兜底源成交量必须 ×100 成「股」、量纲接缝判据不许误报
