...
| **当前主线** | **v6.68 修复轮 + 东财源评估**（`1.51` 已提交；**1.46–1.50 备注重写后待强推**）。**下一步** = `f100` 验证 → 行业列 / §7-B4 / 深色主题。 |
| **断点** | **v6.68（续）** 东财深查结案：`push2` 系=**概率性风控**（非代码/参数/代理）；已补"取不到要出声"。⚠ **待验证** = 行业列改走 `clist` 的 `f100` —— 本轮**未验成**（我的探测把 IP 打进了风控）。坑 = §11.5-98/99。 |
...
- **11.5-77** 【v1.39 · §7-E3】① **"裁列白名单"必须 apply-if-present（只减不增）**：各分区列集合
   ...
   **用 `git worktree add ..\Jian-baseline HEAD` 拉一份干净基线跑→ 同样 816/1** ⇒ 环境/时机敏感的
   ...
   - **11.5-98 / -99**（**数据源认知 · 东财源评估** · v6.68）→ 详情已搬 `docs/JIAN_ARCHIVE.md`
     「三、数据源坑」：① **东财 `push2/push2his` 是"概率性风控"**（四证 + "网页 200 而 API 被 RST、
     **网页登录无改善**"见归档）⇒ **只当"单次请求型第二源"，永不承担"几十次连击"型任务**；② 高频取数换极少请求入口或换源；③ 取不到**必须出声**（已做）；
     ④ ⚠ **探测本身也会把 IP 打进风控** ⇒ 排查省着打、间隔开。候选改法（`f100` 一次取全市场行业）**待验证**。



### 11.6 当前"下一步做什么"的推荐顺序（历史刷新**倒序**排列：主清单之下**第一块就是最新**）

> **v6.68 更新块（复权切换 · 测试隔离 · 备注规范 · 行业不静默 · `1.50`/`1.51`）→ 见 `JIAN_HISTORY.md`**
> 一句话：复权的**空字符串 key 被组件吞掉**（手点无效 ⇒ 断言全绿真人点不动）⇒ 改按索引判；同批修测试
> 隔离两处、两条流程规范、**行业映射取不到必须出声**。坑 = §11.5-96/97/98。
...
- [ ] **改了行情工作台页面（`trading_desk.py`）/ `ui/widgets/desk_*.py` 任一模块 / 回测页「成交模型」行 / 标注交互层 / **画线类型目录（新增类型、`implemented` 翻牌）**？** → 跑 `py tests/smoke_pages_overlay.py`（**719 项**，含 **§7-B6 的「迁移护栏」+ 顶栏分段控件/分钟档位 + 工具行 chips + 图标轨/分页面板/折起（含**富余宽度归图表、折起后左侧只剩图标轨**两条不变量）+ 读数条 + **口径回执的"除权跳空定位 / 数据体检"**+ STEP 6 的"实现落在哪个 `desk_*.py`"**：公共面被改名、旧入口（`cb_period`/`cb_adjust`/`cmb_tool`）被复活、**把薄壳写成空函数**、**分栏比例退化**、**回执退回"不解释"**，都会立刻红）；
...
- [ ] **改了复盘页（`ui/views/review.py`）/ `ui/widgets/review_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**719 项**，含 **「复盘页迁移护栏」+ §9-U 分栏不变量**：
...
- [ ] **改了全市场筛选页（`ui/views/scan_view.py`）/ `ui/widgets/scan_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**719 项**，含 **「M2 公共面护栏」+ 版式与口径不变量**）。
...
- [ ] **改了广度统计页（`ui/views/breadth_view.py`）/ `ui/widgets/breadth_*.py` 任一模块？** → 跑
      `py tests/smoke_pages_overlay.py`（**719 项**，含 **「M3 公共面护栏」+ 双窗格/区间/增量不变量**）。
...
- [ ] **改了回测页/工作台/复盘页的叠层、检测、图层开关、公式对话框、窗格编排、用户标注、配方库/互送、自选股/周期/复权、成交模型行，跑过 `py tests/smoke_pages_overlay.py` 吗？**（**719 项**，页面级；标注与配方一律用**临时库**，脚本末尾还有"用户真实库未被写"的**防污染自检**（现含 `backtest_strategies.json`）；四条离屏打桩见 §11.5-20 与 §11.5-79（★第④条 v1.42 补：**异步回测与后台下载也不许写真目录** —— 看到"只有几条真实数据类断言红"先问“有没有东西正在写”），**别删**）
...
- [ ] **改了后台下载（`ui/download_hub.py` / `ui/widgets/download_bar.py` / `download_queue_panel.py`）或四个下载入口（`bulk_download` / `data_manager` / `readiness_flow` / `main_window.downloads`）吗？** → 跑 `py tests/smoke_pages_overlay.py`（**719 项**，含「§7-B11 后台下载」段）。⚠ 四条红线：
