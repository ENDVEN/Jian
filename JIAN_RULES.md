...
| 项 | 值 |
|---|---|
| APP 版本 | **1.43**（三处同步 §9-A：`APP_VERSION` ✅ / `version.json` ✅ / commit 首词 ✅ `1.43`；**push 待用户**。1.42 与 1.43 **合成一个 commit**，不再单独占号） |
| 文档版本 | **v6.59**（§7-B11 后台下载**收口轮**：两个真 bug + 单只互斥公共件 + 真实线程端到端验证） |
| 最近三版 | `1.43` §7-B11 后台下载（含 1.42 数值控件宽度）· `1.41` §7-B1/B2 补漏 · `1.40` §7-E5 下载层日历感知 |
| **当前主线** | **§7-E 队列已闭环（E1–E5）；§7-B11 后台下载已落地并收口（1.43）**；**push 待用户**（领先远端 8：1.35–1.43）。**下一步候选** = §7-B4 回测进阶（复用 `TradeAnalyzer` 绩效维度 + 每回合明细持久化）/ 深色主题（四个回测子模块已全部收官，**可能已到该时点**）；**⏸ 另有 1 项待拍板** = 下载速度性能档（并发 / 批量快照通道，见 §11.6 顶部） |
| **断点** | **v6.59（1.43）§7-B11 后台下载 ✅ 收口完成**：任务归属从弹窗搬到主窗口（新 `ui/download_hub.py`，串行 K=1、**不自造 QThread**）+ 底部下载条 / 导航角标 / 非模态队列面板三处**投影** + 批量预下载改非模态 + 数据管理页不再锁整页 + M2/M3 交队列。**收口轮修掉两个真 bug**：① `cancel(全部)` 漏给**排队中**任务发 `job_finished` ⇒ 页面永久卡在"进行中"（§11.5-82）；② 下载条「中断」误用 `cancel(None)` ⇒ 静默取消排队任务。**同批收敛**：单只互斥的 token 从 4 处手写收成唯一公共件 `ui/download_hub.SingleSyncGate`；去重键补全 `min_date`/档力度；弹窗 parent 改主窗口；`hub_of` 取件统一。**同批 1.42**：「间隔(秒)」被 64px 钉死截成"0"⇒ 收成 `custom_widgets.double_spin/int_spin` 唯一工厂（§10-9 旧债还清）。验收 `smoke_chart` **817** / `smoke_pages_overlay` **627** 全绿（断言数唯一出处 = §11.7）；另用**真 SyncWorker / 真 QThread**（只打桩 `refresh_one` 与日历注入）跑通 15 项端到端验证（串行 / 跨线程进度 / 中断保留真实进度 / 退出守卫真等线程结束）。
**前序已闭环**：§7-B1/B2 补漏（1.41）/ §7-E5（1.40）/ §7-E3（1.39）/ §7-E2（1.38）/ §7-E4（闸门结案·零代码）/ §7-A4（1.37）—— 逐条叙事见 §8「最近三版」与 `docs/JIAN_HISTORY.md` §8 全表。
**⏸ 待拍板**：下载速度性能档（并发 / 批量快照通道）→ 见 §11.6 顶部。 |
...
- **业务文件 ≥400 行（★v1.43 全量现测；涨落史不抄在这里，只存"现在多长 + 再长怎么拆"）**：
  `ui/views/backtest.py` **940**（再长把"预览回放态"整块搬 `ui/widgets/backtest_preview.py`）>
  ⚠ `ui/widgets/custom_widgets.py` **931**（★v1.43 首次登记：K 线图元 + NoWheel 控件族 +
    复合控件 QSS 生成器 + 数值控件工厂 + 图层 chips…全站样式公共件都在这；**登记不返工**，
    再加新组件前先按"图元 / 控件 / 样式常量 / chip 族"分文件）>
  ...
  ⚠ `data/sync_service.py` **601**（v1.40 日历感知新鲜度 + v1.43 `failure_hint`；
  ...
  ⚠ `ui/views/data_manager.py` **508**（**已破 500**；v1.43 交队列**未净减**（506 → 508），仅把"锁整页"换成"提交队列"；再长拆"分区清单 / 同步操作条"两件）>
  ⚠ `ui/views/trading_desk.py` **499**（★v1.43 纠偏：旧写"345 已退出 400 线"是 1.23 的数，
    1.24/1.34 之后重新越线；再长继续往 `desk_*.py` 外搬）>
  `core/backtest.py` **495** > `data/akshare_feed.py` **487**（再加新源前先分文件）>
  ⚠ `ui/widgets/desk_watch.py` **481** > ⚠ `ui/widgets/breadth_flow.py` **476**（收口时与 `scan_flow`
    一起把"范围解析→体检接线"收进 `ReadinessFlow`）> ⚠ `ui/widgets/backtest_result.py` **478** >
  ⚠ `ui/dialogs/bulk_download.py` **469**（v1.43 交队列后不再持有线程）>
  ⚠ `ui/widgets/backtest_history_ui.py` **484**（再长拆 `backtest_history_preview.py`）>
...
- ✅ **v1.43 新建的三个下载件都没越线**（`ui/download_hub.py` **322** / `download_bar.py` 123 /
  `download_queue_panel.py` 137）—— 按 v6.16 口径不标数字，**新增另起模块、没往弹窗与页面里堆**。
...
| **广度统计 M3** | 逐日"有多少只满足条件"：广度家数**柱状/折线双视觉**（默认柱状+MA5 趋势线，家数刻度取整）+ 指数副图 x 联动（**折线/面积/K线/美国线四图形，按日对齐只画当前区间，v6.43**）+ **两窗格轴对齐 + 分界把手拖动调高**（v6.43）· **⚡增量到最新**（尾段续接，历史一天不动）· **⟳全量重算**（二次确认）· 区间切片零成本 · **就绪度体检 + 「⬆ 更新到最新交易日」**（v6.39/1.35 合并一键；v1.43 交后台队列） | `ui/views/breadth_view.py` + `ui/widgets/breadth_*` + `readiness_flow`（v6.38/39/43） |
...
### B11 · 后台下载（v1.43 已落地）仍现行、必须记住的 4 条口径
...
3. **串行 K=1**：与"宁可慢也不封 IP"一致；并发档另案待拍板（§11.6 顶部）。同样的清单不重复入队。
...
- **A. ✅ 已修（v5.7）版本号滞后；🔁 编号体系于 v6.16 改版（用户拍板）**：
  ...
  · **当前值 = `1.43`**（= §7-B11 后台下载；1.42 = §10-9 数值控件宽度工厂；
...
> ⏸ **待用户拍板（需要你点头才动 —— 别自己决定）**
> **下载速度的"性能档"（v1.41 登记 · 用户 2026-09-23 原话："限制这个下载速度拍板这部分
> 我们放到后面去给我决定"）**：§7-E5 只消除了"周末/节假日/盘中空转全池"的**纯浪费**；
> 剩下三档**都带代价，须拍板**：
> ① **并发 K=3~4**（池只能建在 `ui/workers.py`，数据层保持纯同步）：5400 只 ≈54min → **~14min**；
>    代价 = 与"宁可慢也不封 IP"的既有取向冲突（聚合请求率 ×K）。
> ② **批量快照通道** `ak.stock_zh_a_spot_em()`（1 次请求拿全市场当日 OHLCV+额+换手+市值）：
>    盘后补当天 ≈54min → **~1–2min**；⚠ 三条硬边界：**只能 15:05 定稿后跑**（否则落库半根 bar）、
>    **只能补当天这一根**（多日缺口仍须逐只）、**spot 是不复权价**（不能当复权修正通道）。
> ③ （**不推荐**）降 `interval` / 优化本地 IO —— 前者有封 IP 风险，后者实测只占 2–5%（§11.5-78⑤）。
> **建议**：先在真实场景实测一轮"日常更新到最新"的耗时（v1.41 后应已大幅下降），
> 拿到你自己的数字再决定要不要上 ①/②。

> **v6.59（§7-B11 后台下载 · 收口轮 · `1.43` · 2026-09-23）**
...
>   唯一公共件 `SingleSyncGate`（幂等 hold / 可重复 release）；去重键补 `min_date` 与档力度
>   （原会"同清单不同起点被判重复 ⇒ 第二方范围被静默忽略"）；批量弹窗 parent 改主窗口
>   （原挂页面上，切页可能连窗一起消失）；`readiness_flow` 取件统一走 `hub_of` 并降级。
> · **新增验证层**：**真 SyncWorker / 真 QThread** 端到端探针（只打桩 `refresh_one` 与日历注入）
>   15 项全过 —— 串行不断链 / 跨线程进度 / 中断保留真实进度(2/5) / 退出守卫真等到线程结束。
>   ⚠ 这正是**纯组件与页面级冒烟都测不到**的一层（假线程永远按时回包）。
> 验收：`smoke_pages_overlay` **+16 项**（覆盖两个 bug 与 `SingleSyncGate`，另有源码级断言
> 钉"只有 hub 里能出现 `note_single(`"）、`smoke_chart` 不变；当前条数见 **§11.7**。
> 坑 = **§11.5-82**；遗留三笔（退出等待无反馈 / 无"继续未完成" / `FLAT_QSS` 归属）→ **§9-F**。
> 样板（未变）`design/1.43-bg-download/index.html`。
...
> **v6.49–v6.56 的刷新块 → 已搬 `docs/JIAN_ARCHIVE.md`「三、§11.6 历史刷新块」**（v6.59 按 §10-14 搬走）。
> 一句话回顾：`1.37` 运行历史子页 · `1.38` 代理文案 + 熔断 · `1.39` 日线列白名单 · `1.40` 下载层日历感知 · `1.41` M2/M3 入口完整性 · `1.42` 数值控件宽度 · `1.43` 后台下载；逐条叙事仍见 `docs/JIAN_HISTORY.md` §8 全表。
